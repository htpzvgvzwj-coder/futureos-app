...
import { buildStage2SystemPrompt } from "../../../../lib/other-prompts.js";
import { FINALIZE_SAVINGS_PLAN_TOOL, PROPOSE_SAVINGS_PLAN_TOOL } from "../../../../lib/other-tools.js";
import { finalizeSavingsPlanSchema, proposeSavingsPlanSchema } from "../../../../lib/other-validation.js";
import { buildMockSavingsFinalization, buildMockSavingsPlanOptions, looksLikeConfirmation } from "../../../../lib/other-mock.js";
import {
  appendMessages,
  getLatestArtifact,
  getMessageHistory,
...
// Same mock fallback pattern as stage1 - only reached when the real Anthropic call itself fails.
async function buildMockToolUse(message, sessionId, confirmedPlan, profile) {
  ...
  if (looksLikeConfirmation(message) && previousStrategyOptions) {
    return {
      ...
      input: buildMockSavingsFinalization(message, previousStrategyOptions, confirmedPlan),
    };
  }
  return { name: "propose_savings_plan", input: buildMockSavingsPlanOptions(confirmedPlan, profile) };
}
...
export async function POST(request) {
  ...
  const confirmedPlan = await getLatestArtifact(session.id, "stage1", "confirmed_goal_plan");
  if (!confirmedPlan) {
  ...
  try {
    const response = await runToolTurn(client, {
      ...
      system: buildStage2SystemPrompt(language, resolvedProfile, confirmedPlan),
    ...
    console.error("other/stage2 Anthropic call failed, falling back to mock response", error);
  ...
  if (!parsed.success) {
    console.error("other/stage2 tool output failed validation", parsed.error.issues);
