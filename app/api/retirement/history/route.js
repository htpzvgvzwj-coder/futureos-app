import { getMessageHistory, getOrCreateSession } from "../../../../lib/retirement-store.js";
import { resolveEffectiveProfileKey } from "../../../../lib/auth.js";

export const runtime = "nodejs";

const TOOL_SUMMARIES = {
  propose_retirement_plans: (input) => `Presented ${input.plans?.length ?? 0} retirement lifestyle option(s).`,
  confirm_retirement_plan: (input) => `Confirmed retirement plan: ${input.plan_id}.`,
  propose_retirement_savings_plan: (input) => `Presented ${input.strategies?.length ?? 0} savings strategy option(s).`,
  finalize_retirement_savings_plan: (input) => `Confirmed savings plan: SGD ${input.monthly_contribution}/month.`,
};
...
export async function GET(request) {
  const resolved = await resolveEffectiveProfileKey(request, "retirement");
