...
import { buildMockSavingsFinalization, buildMockSavingsPlanOptions, looksLikeConfirmation } from "../../../../lib/retirement-mock.js";
import {
  appendMessages,
  getLatestArtifact,
  getMessageHistory,
...
async function buildMockToolUse(message, sessionId, confirmedPlan, profile) {
  ...
  return { name: "propose_retirement_savings_plan", input: buildMockSavingsPlanOptions(confirmedPlan, profile) };
}
...
export async function POST(request) {
  ...
  // "flexible" horizon - decades of runway, plenty of time to plan
  // liquidating a market-exposed holding. See lib/asset-finance.js's
  ...
  try {
    ...
    toolUse = findToolUse(response.content, ["propose_retirement_savings_plan", "finalize_retirement_savings_plan"]);
    if (!toolUse) {
