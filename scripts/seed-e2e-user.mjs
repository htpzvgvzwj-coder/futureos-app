// Seed a dedicated, deterministic E2E user with a reality path for every
// one of the nine Studios, then write a Playwright storageState so the E2E
// suite never has to skip for a missing login.
//
//   npm run test:e2e:seed        # once (needs DATABASE_URL + the app's auth)
//   npm run dev                  # app on 127.0.0.1:3000
//   npm run test:e2e
//
// Idempotent: re-running re-keys nothing new; it refreshes the session
// token + storageState.
...
const E2E_EMAIL = process.env.E2E_USER_EMAIL ?? "e2e@futureos.test";
const E2E_PASSWORD = process.env.E2E_USER_PASSWORD ?? "e2e-password-not-secret";
const BASE_URL = process.env.E2E_BASE_URL ?? "http://127.0.0.1:3000";

// A minimal, honest reality path per Studio (real numbers, no demo copy).
const REALITY = {
...
const existing = await query(`select id, email from users where email = $1`, [E2E_EMAIL]);
const user = existing.rows[0] ?? (await createUser({ email: E2E_EMAIL, password: E2E_PASSWORD, displayName: "E2E" }));
console.log(`E2E user: ${user.email} (${user.id})`);

for (const [domain, patch] of Object.entries(REALITY)) {
  ...
  if (versions.length === 0) {
    ...
    console.log(`  ${domain}: seeded reality path`);
  } else {
    console.log(`  ${domain}: reality path already present (${versions.length} version(s))`);
  }
}
...
const outFile = path.join(ROOT, "e2e/.auth/user.json");
writeFileSync(outFile, JSON.stringify(storageState, null, 2) + "\n");
console.log(`\nstorageState -> ${path.relative(ROOT, outFile)}`);
console.log(`session cookie ${SESSION_COOKIE_NAME}=${token.slice(0, 12)}...  (expires ${expiresAt})`);
console.log(`\nNow: npm run dev  (on ${BASE_URL})  then  npm run test:e2e`);
process.exit(0);
