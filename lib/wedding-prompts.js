...
Use the web_search tool to ground your cost estimates in current Singapore wedding market pricing (venues, catering per head, photography, attire, and any other categories relevant to what the customer described). Every line item must include an "estimate_basis" explaining what informed that number — never present an unexplained number.
...
Keep each plan to at most 8 line_items and at most 10 timeline entries — comprehensive but not exhaustive, so the response stays a reasonable size. If the customer's requirements are underspecified, still propose 2-3 plans using reasonable assumptions and state those assumptions in research_notes rather than asking a clarifying question in plain text.
