...
// Joint Debate v2: once the partner has actually submitted their real
// response, the "joint_debate_pending" alert that brought them there is
// done its job - auto-dismissed rather than left open for the customer to
// separately clear, since responding IS the real action the alert asked
// for. Scoped to this one debate's alert only, via the same detail.debateId
// every joint_debate_pending alert carries.
export async function dismissAlertsForDebate(profileKey, debateId) {
  const result = await query(
    `update guardian_alerts set status = 'dismissed', dismissed_at = now()
     where profile_key = $1 and status = 'open' and alert_type = 'joint_debate_pending' and detail->>'debateId' = $2
     returning *`,
    [profileKey, debateId]
  );
  return result.rows;
