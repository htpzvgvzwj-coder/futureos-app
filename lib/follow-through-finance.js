...
// Dimension 5 (10%): judgment/calibration, not compliance - did the customer's
// OWN stated reason for proceeding despite a Mirror-flagged risk (their
// rebuttal, lib/mirror-store.js customer_rebuttal) hold up against the same
// real 90-day resolution mirror-outcome-resolver.js already runs for the
// AI's own accountability (Guardian Reputation Score's predictiveAccuracy).
// Excluded (not scored 0) until the customer has actually written a
// rebuttal AND it has resolved - most customers will have no data here for
// a long time, same "insufficient data is excluded" pattern as every other
// dimension above.
function computeJudgmentCalibration(calibrationStats) {
  if (!calibrationStats || calibrationStats.resolvedCount === 0) {
    return { value: null, resolvedCount: 0, heldUpCount: 0 };
  }
  return {
    value: clamp(Math.round((calibrationStats.heldUpCount / calibrationStats.resolvedCount) * 100)),
...
const WEIGHTS = {
  ...
  amountFidelity: 0.3,
  recoveryHonesty: 0.15,
  multiGoalDepth: 0.1,
  judgmentCalibration: 0.1,
};

export function computeFollowThroughScore({ domains, hardshipEvidence, everAtRisk, confirmedGoalCount, calibrationStats }) {
  const components = {
    ...
    judgmentCalibration: { ...computeJudgmentCalibration(calibrationStats), weight: WEIGHTS.judgmentCalibration },
  };
