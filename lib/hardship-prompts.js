...
export function buildHardshipRecoverySystemPrompt(language, { assessment, computed, isIncomeIrregular, incomeSampleSize }) {
  const languageName = LANGUAGE_NAMES[language] ?? LANGUAGE_NAMES.en;
...
Real monthly income gap: SGD ${computed.gap.monthlyShortfall} (using ${assessment.stated_new_monthly_income != null ? "the customer's stated new income of SGD " + assessment.stated_new_monthly_income : "their existing income, since no new figure was stated"}).${isIncomeIrregular && assessment.stated_new_monthly_income == null ? ` Note: this "existing income" is itself a ${incomeSampleSize}-month smoothed median - this customer's income genuinely varies month to month, which is real context for how tight or loose this gap actually is.` : ""}
