...
// Real "how much could I realistically borrow" preview for renovation/
// personal (unsecured) purposes, shown BEFORE the customer states a loan
// amount - there's no benchmark for "a renovation costs X" the way there's
// no benchmark for wedding cost, but there IS a real serviceability ceiling
// this app already knows. Reuses the exact same ceiling/target-payment math
// computeLoanArchetype's "balanced" archetype uses, so this preview is
// never a separate guess from what the archetype step computes once a real
// amount is confirmed - just the same math run one step earlier.
export function computeLoanAffordabilityPreview({ purposeKey, monthlyIncome, otherGoalsMonthlyOutflow = 0 }) {
  const purpose = LOAN_PURPOSES[purposeKey];
  const tenureYears = purpose.archetypeTenureYears.balanced;
  const monthlyCeiling = calculateServiceabilityCeiling({ monthlyIncome, existingMonthlyDebt: otherGoalsMonthlyOutflow, msrApplies: false });
  const maxLoanAmount = calculateLoanForTargetPayment(monthlyCeiling, purpose.baseRatePercent, tenureYears);
  return {
    maxLoanAmount: Math.round(Math.max(0, maxLoanAmount)),
    monthlyCeiling: Math.round(Math.max(0, monthlyCeiling)),
    tenureYears,
  };
}
