...
async function getDomainCommitment(domain, profileKey) {
...
export async function getCustomerCommitments(profileKey) {
  const [wedding, home, retirement] = await Promise.all([
    getDomainCommitment("wedding", profileKey),
    getDomainCommitment("home", profileKey),
    getDomainCommitment("retirement", profileKey),
  ]);
...
export function getEmergencyFundSnapshot(profile) {
  return {
    ...
    isIncomeIrregular: Boolean(profile.isIncomeIrregular),
    incomeSampleSize: profile.incomeSampleSize ?? 0,
  };
}
