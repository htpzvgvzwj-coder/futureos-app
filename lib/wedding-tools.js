...
const lineItemSchema = {
  ...
  properties: {
    category: {
      type: "string",
      description:
        "e.g. car, makeup, entertainment, favors, solemnization, decor, stationery, misc activities — NOT venue, photography, or attire, which the app calculates separately from venue_tier/venue_type/photography_tier/attire_tier",
    },
    label: { type: "string" },
...
const planSchema = {
  ...
  properties: {
    ...
    venue_tier: {
      ...
      description: "Has no cost effect when venue_type is 'community' — default to 'mid_range' in that case.",
    },
    venue_type: { type: "string", enum: ["hotel", "restaurant", "community"] },
    photography_tier: { type: "string", enum: ["basic", "mid_range", "premium"] },
    attire_tier: { type: "string", enum: ["budget", "mid_range", "premium"] },
...
export const PROPOSE_PLANS_TOOL = {
  name: "propose_plans",
  description:
    "Present 2-3 distinct complete wedding plan options to the customer for comparison, each with an itemized cost breakdown and a day-of activity timeline. Venue, photography, and attire cost is calculated by the app from venue_tier/venue_type/photography_tier/attire_tier — do not compute or estimate those dollar amounts yourself, and do not include them in line_items.",
  strict: true,
  input_schema: {
    type: "object",
    properties: {
      plans: { type: "array", items: planSchema },
      research_notes: { type: "string", description: "General notes on the market research behind the remaining (non-venue/photography/attire) line item estimates." },
    },
...
export const CONFIRM_WEDDING_BUDGET_TOOL = {
  ...
  description:
    "Call this ONLY when the customer has unambiguously confirmed one specific final plan. Carries the final itemized budget, activities, and timeline that will be persisted and handed off to the savings-plan stage. Venue, photography, and attire cost is calculated by the app from venue_tier/venue_type/photography_tier/attire_tier — do not compute or estimate those dollar amounts yourself, and do not include them in line_items.",
  strict: true,
  input_schema: {
    type: "object",
    properties: {
