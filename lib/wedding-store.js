...
// Atomic upsert, not check-then-insert - two concurrent first-ever requests
// for the same new profileKey (real scenario: lib/open-loops.js now calls
// this across multiple domains via Promise.all) would otherwise both see no
// existing row and both attempt the insert, violating the unique index.
// "do update set profile_key = excluded.profile_key" is a no-op write, only
// there because ON CONFLICT DO NOTHING wouldn't RETURNING the existing row.
export async function getOrCreateSession(profileKey) {
  const result = await query(
    `insert into wedding_sessions (profile_key) values ($1)
...
export async function saveArtifact(sessionId, stage, artifactType, payload) {
  const result = await query(
    "insert into wedding_artifacts (session_id, stage, artifact_type, payload) values ($1, $2, $3, $4) returning created_at",
...
export async function getLatestArtifactWithTimestamp(sessionId, stage, artifactType) {
  const result = await query(
    `select payload, created_at from wedding_artifacts
     where session_id = $1 and stage = $2 and artifact_type = $3
     order by created_at desc limit 1`,
    [sessionId, stage, artifactType],
  );
