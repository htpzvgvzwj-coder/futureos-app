...
// The uuid-typed FK-ish columns (commitment_id / plan_id / plan_branch_id /
// supersedes_event_id) must be a real uuid or null - a malformed id from a
// caller should degrade the event gracefully (drop that one reference), not
// abort the whole ledger write with a Postgres type error.
export function uuidOrNull(value) {
  if (value == null) return null;
  const s = String(value);
  if (UUID_RE.test(s)) return s;
  console.warn(`[change-ledger] ignoring non-uuid id: ${s.slice(0, 40)}`);
  return null;
}
...
export async function recordEvent(draft, { client = null } = {}) {
  ...
  const params = [
    ...
    draft.actionType,
    draft.status,
    uuidOrNull(draft.planId),
    uuidOrNull(draft.planBranchId),
    uuidOrNull(draft.commitmentId),
    ...
    uuidOrNull(draft.supersedesEventId),
    draft.messageKey,
