...
export function computeGoalFeasibility(goalType, inputs, assetContext = {}, overrides = {}) {
  ...
  const { affordabilityRatio, feasibilityScore, riskLevel } = computeAffordabilityScore(availableMonthly, requiredMonthly);
...
// The affordability-ratio-to-score formula at the heart of
// computeGoalFeasibility above, extracted so other real callers with a
// different input shape (lib/joint-plan-evidence.js - a goal-plan joint
// confirm's monthly_contribution, not Mirror's target-amount+date or
// direct-monthly fields) can reuse the exact same real math instead of a
// second, potentially-diverging formula. Centered on ratio 1.0 (exactly
// enough) at score 50, +/-30 points per whole ratio unit away from that,
// clamped to a 20-96 band so nothing ever reads as impossible (0) or
// risk-free (100).
export function computeAffordabilityScore(availableMonthly, requiredMonthly) {
  const affordabilityRatio = requiredMonthly > 0 ? availableMonthly / requiredMonthly : 1;
  ...
  return { affordabilityRatio: Math.round(affordabilityRatio * 100) / 100, feasibilityScore, riskLevel };
}
