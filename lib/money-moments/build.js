...
function rescueActions(c) {
  return (c.options ?? []).map((o) => {
    switch (o.id) {
      case "recognise":
        return { id: "acknowledge", label: "I recognise this", route: null, available: true };
      case "dispute":
        return { id: "dispute", label: "I don't recognise this", route: null, available: false, unavailableReason: "No connected bank dispute rail" };
      case "retry":
      case "update_source":
      case "contact_biller":
        return { id: o.id, label: o.label, route: null, available: false, unavailableReason: "External payment rail not connected" };
      case "review":
      case "recategorise":
        return { id: o.id, label: o.label, route: "today:activity", available: true };
      case "open_mirror":
      case "reduce_contribution":
      case "pause_one":
      case "reduce_all":
        return { id: o.id, label: o.label, route: "home", available: true };
      case "open_future_balance":
        return { id: o.id, label: o.label, route: "today", available: true };
      default:
        return { id: o.id, label: o.label, route: "explore", available: true };
    }
  });
}
...
function rescueToMoment(c, asOf) {
  ...
  return {
    ...
    whyNow: RESCUE_WHY_NOW[c.kind] ?? "Flagged from your latest ledger and plan data.",
    evidence,
    moneyChange: null,
...
function driftToMoment(driftCase, drift, asOf) {
  ...
  return {
    ...
    state: "new",
    nextActions: [
      { id: "accept_new_reality", label: "Update the plan to match reality", route: "home", available: true },
      { id: "keep_original_plan", label: "Keep the original plan", route: null, available: true },
    ],
    occurredAt: asOf,
...
function rippleToMoment(e, asOf) {
  ...
  return {
    ...
    nextActions: [
      { id: "view_cause", label: "See what caused this", route: "history", available: true },
      ...(e.domain ? [{ id: "review_plan", label: `Review your ${e.domain} plan`, route: `studio:${e.domain}`, available: true }] : []),
    ],
...
function ledgerToMoment(e, asOf) {
  const headline = formatEvent(e, (k) => k)?.headline ?? e.message_key;
  return {
    id: `ledger:${e.id}`,
    sourceType: "confirmed_change",
    ...
    affectedPlans: [],
    state: "new",
    nextActions: [{ id: "view_history", label: "Open full history", route: "history", available: true }],
...
function turningPointToMoment(tp, asOf) {
  ...
  return {
    ...
    affectedPlans: [],
    state: "new",
    nextActions: [{ id: "open_guardian", label: "Review with Guardian", route: "guardian", available: true }],
    occurredAt: asOf,
  ...
  for (const grp of si.aggregated ?? []) {
    ...
    if (grp.favourable === false && grp.state !== "confirmed" && grp.possibleDelta != null) {
      out.push({
        ...
        state: "new",
        nextActions: [{ id: "review_impact", label: "Review this plan's impact", route: "explore:plans", available: true }],
        occurredAt: asOf,
