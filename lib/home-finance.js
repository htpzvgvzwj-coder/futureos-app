...
// loan that fits it at this rate/tenure.
function maxLoanForMonthlyPayment(monthlyPayment, annualRatePercent, tenureYears) {
  const monthlyRate = annualRatePercent / 100 / 12;
  const numPayments = tenureYears * 12;
...
export function calculateMaxLoan({
  ...
  const maxLoan = maxLoanForMonthlyPayment(monthlyPaymentCeiling, annualRatePercent, tenureYears);
  return {
...
export function calculateDownPayment(price, { existingLoanCount = 0 } = {}) {
  ...
  // HDB loan: min 20% down payment can be entirely CPF/cash. Bank loan:
  // min 5% of the down payment must be cash, the rest can be CPF/cash.
  // We surface both splits and let the caller pick based on loan_type.
  return {
    ltv,
    loanAmount,
    totalDownPayment,
    minCashComponent: Math.round(price * 0.05),
  };
}
...
export function computeHomeFinancials({
  ...
  const downPayment = calculateDownPayment(price, { existingLoanCount: existingPropertyCount });
