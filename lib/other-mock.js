...
const CONFIRM_PATTERN = /\b(confirm|go with|let'?s do|sounds good|that works|final(ize)?|yes,? (plan|that)|i'?ll take|lock (it|that) in)\b/i;
...
export function buildMockGoalConfirmation(message, previousPlanOptions) {
  ...
  const index = letterMatch ? "abc123".indexOf(letterMatch[1].toLowerCase()) % plans.length : 0;
  const chosen = plans[Math.max(0, index)] ?? plans[0];
...
export function looksLikeConfirmation(message) {
  return CONFIRM_PATTERN.test(message);
}
...
export function buildMockSavingsFinalization(message, previousStrategyOptions, confirmedPlan) {
  ...
  const letterMatch = message.match(PLAN_LETTER_PATTERN);
  const index = letterMatch ? "abc123".indexOf(letterMatch[1].toLowerCase()) % strategies.length : 0;
  const chosen = strategies[Math.max(0, index)] ?? strategies[0];
