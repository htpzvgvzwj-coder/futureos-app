...
export function detectRealityDrift({ planned = {}, observed = {}, thresholds = {}, currency = "SGD", monthsPerImpactUnit = null } = {}) {
  ...
  const check = (metric, plannedVal, observedVal, worseWhenHigher) => {
    ...
    const impactClause = impactMonths
      ? ` If unchanged, a linked goal may move about ${impactMonths} month${impactMonths === 1 ? "" : "s"} ${worse ? "later" : "earlier"}.`
      : "";
    cases.push({
      metric,
      planned: round2(p),
      ...
      summary:
        ...
        impactClause,
      // localisable form: the base sentence is a template; the (rare) impact
      // tail is passed as a pre-formed {impact} slot.
      summaryKey: "Your plan assumed {cur} {p} {metric}. Your {months}-month observed average is {cur} {o}.{impact}",
      summaryParams: { cur: currency, p: round2(p), metric: metric.replace(/_/g, " "), months: monthsObserved, o: round2(o), impact: impactClause },
