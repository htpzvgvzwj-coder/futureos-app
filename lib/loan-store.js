...
// identical fix for why (a real race, not theoretical). Conflict target is
// the compound (profile_key, purpose) unique index, not profile_key alone -
// a customer can have a confirmed home loan AND a confirmed personal loan.
export async function getOrCreateSession(profileKey, purpose) {
  const result = await query(
    `insert into loan_sessions (profile_key, purpose) values ($1, $2)
     on conflict (profile_key, purpose) do update set profile_key = excluded.profile_key
     returning *`,
    [profileKey, purpose],
  );
