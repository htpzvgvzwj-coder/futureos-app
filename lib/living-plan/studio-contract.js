...
export function buildImpactSet({ cause, resourceDelta = {}, affectedGoals = [], allocationRequired = false, assumptions = [], legs = null }) {
  return {
    cause: cause ?? null,
    resourceDelta: {
      ...resourceDelta,
    ...
    affectedGoals: affectedGoals.map((g) => {
      ...
      const confirmedAfter = legs
        ? legConfirmed(legs, g.goalId)
          ? possibleAfter
          : null
        : g.confirmedAfter ?? null;
      return {
        goalId: g.goalId,
