...
// }
export function buildImpactSet({ cause, resourceDelta = {}, affectedGoals = [], allocationRequired = false, assumptions = [] }) {
  return {
    cause: cause ?? null,
    resourceDelta: {
      ...resourceDelta,
      freedMonthly: Math.max(0, Math.round(Number(resourceDelta.freedMonthly) || 0)),
      addedPressureMonthly: Math.max(0, Math.round(Number(resourceDelta.addedPressureMonthly) || 0)),
    },
    affectedGoals: affectedGoals.map((g) => ({
      goalId: g.goalId,
      metric: g.metric ?? null,
      before: g.before ?? null,
      // A POSSIBLE impact (ghost). Solid only once the customer allocates.
      possibleAfter: g.possibleAfter ?? null,
      confirmedAfter: g.confirmedAfter ?? null,
      direction: g.direction ?? "flat", // up | down | flat
...
export function validateImpactSet(impactSet) {
  const errors = [];
