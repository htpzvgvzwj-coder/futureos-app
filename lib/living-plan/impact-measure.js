...
export function aggregateImpactMeasures(measures = []) {
  ...
  for (const raw of measures) {
    ...
    const g = groups.get(k) ?? {
      ...
      deltaSum: 0,
      // confirmed effects are aggregated as DELTAS off the canonical
      // before, never as a sum of absolute confirmedAfter values.
      confirmedDeltaSum: 0,
      hasConfirmed: false,
      count: 0,
      sources: new Set(),
    ...
    if (m.confirmedAfter != null) {
      g.confirmedDeltaSum += Number(m.confirmedAfter) - Number(m.before ?? g.before ?? 0);
      g.hasConfirmed = true;
    }
    g.count += 1;
