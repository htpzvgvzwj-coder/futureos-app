...
export function aggregateImpactMeasures(measures = []) {
  ...
  for (const raw of measures) {
    ...
    if (m.snapshotId != null) g.snapshotIds.add(m.snapshotId);
    ...
    if (m.resourceId) {
      // the same resource must not land in one (goal, metric, unit) group
      // twice - that is the same money counted more than once.
      if (g.resourceIds.has(m.resourceId)) g.duplicateResource = true;
      g.resourceIds.add(m.resourceId);
    }
    g.count += 1;
  ...
  const aggregated = [...groups.values()].map((g) => {
    ...
    // A group whose measures disagree on the baseline cannot be reduced to
    // one number - refuse it outright rather than emit a pseudo-precise
    // after (Part B). No possibleAfter / confirmedAfter is produced.
    const conflictReason = g.beforeMismatch
      ? "baseline_mismatch"
      : g.snapshotIds.size > 1
        ? "baseline_mismatch"
        : g.duplicateResource
          ? "duplicate_resource"
          : null;
    if (conflictReason) {
      return {
        ...
        beforeMismatch: g.beforeMismatch,
        snapshotIdCount: g.snapshotIds.size,
  ...
  const conflictGroups = aggregated.filter((g) => g.state === "conflict");
  return {
    aggregated,
    invalid,
    groupCount: aggregated.length,
    conflictGroups,
    hasBaselineConflict: conflictGroups.length > 0,
  };
}
