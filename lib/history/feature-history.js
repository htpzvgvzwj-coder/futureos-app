...
const FEATURES = {
  ...
  explore: { ledger: ["home", "wedding", "retirement", "loan", "investment", "travel", "family", "emergency"], audit: [] },
  family: {
    ledger: [],
    audit: [
  ...
  if (cfg.ledger.length) {
    const r = await query(
      `select occurred_at, actor, action_type, impact_set, uncertainty_note, cause
        ...
        where profile_key = $1 and visibility <> 'system' and source_feature = any($2)
        order by occurred_at desc limit 40`,
      [profileKey, cfg.ledger],
    );
