...
// named default ratio for a reduce - an explainable rule rather than
// something the model invents), and "the dollar amount moved" for
// drawdown/invest.
export const REDUCE_GOAL_PLAN_RATIO = 0.5;
  ...
  const recoveryAction = z
    ...
    .transform((action) => {
      ...
      if (action.action_type === "reduce_goal_plan") {
        ...
        return { ...action, amount: Math.round((current * REDUCE_GOAL_PLAN_RATIO) / 10) * 10 };
      }
      // Only pause/reduce carry a real target_domain - strip whatever the
      // model put there for every other action_type rather than trusting it,
      // since a hallucinated domain would otherwise flow untouched into the
      // saved artifact and the applied-actions audit trail.
      if (action.action_type === "drawdown_emergency_fund") {
        return { ...action, target_domain: null, amount: defaultDrawdown.suggested };
      }
      if (action.action_type === "invest_excess") {
        return { ...action, target_domain: null, amount: windfallSplit.excessAvailable };
      }
      return { ...action, target_domain: null, amount: 0 }; // other_ocbc_support carries no dollar figure
    });
