...
export async function loadDomainContext(profileKey, domain) {
  ...
  if (domain === "home") {
    ...
    for (const purpose of Object.keys(LOAN_PURPOSES)) {
      const s = await loanStore.getOrCreateSession(profileKey, purpose);
      ...
      if (loan) {
        ...
        realityPlanData = {
          ...
          annual_rate_percent: num(loan.annual_rate_percent),
          tenure_years: num(loan.tenure_years),
          monthly_installment: num(loan.monthly_installment),
          extra_repayment: 0,
          monthly_income: monthlyIncome,
          monthly_expenses: monthlyExpenses,
          current_savings: assetContext.availableLiquidSavings,
          other_goals_monthly_outflow: Math.round(crossGoal.committedMonthlyTotal),
        };
        break;
      }
    }
  } else if (domain === "retirement") {
    const s = await retirementStore.getOrCreateSession(profileKey);
    [confirmedPlan, confirmedSavingsPlan] = await Promise.all([
      retirementStore.getLatestArtifact(s.id, "stage1", "confirmed_plan"),
      retirementStore.getLatestArtifact(s.id, "stage2", "confirmed_savings_plan"),
    ]);
    realityPlanData = confirmedPlan
      ? {
          target_monthly_income: num(confirmedPlan.target_monthly_income),
          gap_monthly: num(confirmedPlan.gap_monthly),
          monthly_contribution: num(confirmedSavingsPlan?.monthly_contribution),
          current_savings: assetContext.availableLiquidSavings,
          monthly_income: monthlyIncome,
          monthly_expenses: monthlyExpenses,
    ...
    const s = await travelStore.getOrCreateSession(profileKey);
    confirmedPlan = await travelStore.getLatestArtifact(s.id, "stage1", "confirmed_budget");
    realityPlanData = confirmedPlan
      ? {
          destination_type: confirmedPlan.destination_type ?? "regional",
          comfort_tier: confirmedPlan.comfort_tier ?? "mid",
          travellers: num(confirmedPlan.travellers) || num(confirmedPlan.guest_count) || 2,
          nights: num(confirmedPlan.nights) || 7,
          trip_month: confirmedPlan.trip_month ?? confirmedPlan.travel_date ?? null,
          total_budget: num(confirmedPlan.total_budget) || null,
          monthly_contribution: num(confirmedPlan.monthly_contribution),
          current_savings: assetContext.availableLiquidSavings,
        }
      : null;
  } else if (domain === "insurance") {
    // No confirmed artifact - the envelope is computed from what the
    // customer has declared on their profile. Unknowns stay unknown.
    ...
    realityPlanData =
      monthlyExpenses > 0
        ? {
            ...
            home_loan_outstanding: num(p.homeLoanOutstanding) || null,
            dependents: p.dependents != null ? num(p.dependents) : null,
            annual_care_cost: num(p.annualCareCost) || null,
            existing_income_protection: p.existingIncomeProtection != null ? num(p.existingIncomeProtection) : null,
            existing_life_cover: p.existingLifeCover != null ? num(p.existingLifeCover) : null,
            existing_ci_cover: p.existingCiCover != null ? num(p.existingCiCover) : null,
            monthly_premium_now: num(p.monthlyInsurancePremium),
          }
        : null;
  } else if (domain === "family") {
    const p = preferences?.profile ?? {};
    realityPlanData =
      num(p.familySharedMonthly) > 0 || Array.isArray(p.familyItems)
        ? {
            shared_monthly_contribution: num(p.familySharedMonthly),
            partner_share_ratio: p.familyShareRatio != null ? num(p.familyShareRatio) : 0.5,
            items: Array.isArray(p.familyItems) ? p.familyItems : [],
          }
        : null;
  } else if (domain === "investment") {
    const s = await investmentStore.getOrCreateSession(profileKey);
    confirmedPlan = await investmentStore.getLatestArtifact(s.id, "stage1", "confirmed_investment_pick");
    realityPlanData = confirmedPlan
      ? {
          monthly_commitment: num(confirmedPlan.amount),
          horizon_years: num(confirmedPlan.horizon_years) || 10,
          target_pool: num(confirmedPlan.projection?.projectedEndValue) || null,
          current_savings: assetContext.availableLiquidSavings,
          monthly_expenses: monthlyExpenses,
          credit_card_outstanding: num(preferences?.profile?.creditCardOutstanding),
          available_monthly_cashflow: monthlyIncome > 0 ? monthlyIncome - monthlyExpenses - crossGoal.committedMonthlyTotal : 0,
          monthly_income: monthlyIncome,
        }
      : null;
  }
