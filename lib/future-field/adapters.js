...
import { projectWeddingThreadImpact } from "../wedding/wedding-thread-projector.js";
import { WEDDING_RATE_PROVENANCE } from "../wedding/rate-provenance.js";
import { computeFutureScore } from "../loan-finance.js";
import { monthsToPayoff as monthsToPayoffLoan } from "../living-plan/monthly-shift-projection.js";
...
import { computeSafetyRunway } from "../emergency/runway-finance.js";
import { projectRunwayImpact } from "../emergency/runway-projector.js";
import { computeDebtGravity } from "../loan/debt-gravity-finance.js";
import { projectDebtImpact } from "../loan/debt-gravity-projector.js";
import { computeFutureLoom, requiredContributionForFutureDay } from "../retirement/future-day-finance.js";
import { projectFutureDayImpact } from "../retirement/future-day-projector.js";
import { computeCalendarOrbit } from "../travel/calendar-orbit-finance.js";
import { projectCalendarOrbitImpact } from "../travel/calendar-orbit-projector.js";
import { computeCapitalPrism } from "../investment/capital-prism-finance.js";
import { projectCapitalPrismImpact } from "../investment/capital-prism-projector.js";
import { computeLivingEnvelope } from "../insurance/living-envelope-finance.js";
import { projectLivingEnvelopeImpact } from "../insurance/living-envelope-projector.js";
import { computePrivateConstellation } from "../family/private-constellation-finance.js";
import { projectPrivateConstellationImpact } from "../family/private-constellation-projector.js";

// family planData -> the Private Constellation plan + context.
const CONSTELLATION_PLAN_KEYS = ["shared_monthly_contribution", "partner_share_ratio", "items", "participants", "monthly_income", "monthly_expenses", "minimum_current_breathing_room"];
function constellationPlan(reality, branch = {}) {
  const out = {};
  for (const k of CONSTELLATION_PLAN_KEYS) {
    if (branch[k] != null) out[k] = branch[k];
    else if (reality[k] != null) out[k] = reality[k];
  }
  return out;
}
function constellationContext(reality, context = {}) {
  return {
    monthlyIncome: Number(context.monthlyIncome ?? reality.monthly_income) || 0,
    monthlyExpenses: Number(context.monthlyExpenses ?? reality.monthly_expenses) || 0,
    otherGoalsMonthlyOutflow: Number(context.committedExcludingDomain ?? context.otherGoalsMonthlyOutflow ?? context.committedMonthlyTotal) || 0,
    now: context.now ?? undefined,
  };
}

// insurance planData -> the Living Envelope plan + context.
const ENVELOPE_PLAN_KEYS = ["monthly_premium_now", "income_protection_months", "existing_income_protection", "existing_life_cover", "existing_ci_cover", "home_loan_outstanding", "dependents", "annual_care_cost", "care_years", "years_of_support_per_dependent", "monthly_expenses", "monthly_income", "desired_cover", "minimum_current_breathing_room", "minimum_income_protection_months"];
function envelopePlan(reality, branch = {}) {
  const out = {};
  for (const k of ENVELOPE_PLAN_KEYS) {
    if (branch[k] != null) out[k] = branch[k];
    else if (reality[k] != null) out[k] = reality[k];
  }
  return out;
}
function envelopeContext(reality, context = {}) {
  return {
    monthlyIncome: Number(context.monthlyIncome ?? reality.monthly_income) || 0,
    monthlyExpenses: Number(context.monthlyExpenses ?? reality.monthly_expenses) || 0,
    otherGoalsMonthlyOutflow: Number(context.committedExcludingDomain ?? context.otherGoalsMonthlyOutflow ?? context.committedMonthlyTotal) || 0,
...
// investment planData -> the Capital Prism plan + context.
const PRISM_PLAN_KEYS = ["jobs", "monthly_commitment", "liquidity_gate_years", "horizon_years", "target_pool", "current_savings", "credit_card_outstanding", "monthly_income", "monthly_expenses", "real_return_assumption"];
function prismPlan(reality, branch = {}) {
  const out = {};
  for (const k of PRISM_PLAN_KEYS) {
    if (branch[k] != null) out[k] = branch[k];
    else if (reality[k] != null) out[k] = reality[k];
  }
  return out;
}
function prismContext(reality, context = {}) {
  return {
...
// travel planData -> the Calendar Orbit plan + context.
const TRAVEL_ORBIT_KEYS = ["travellers", "nights", "comfort_tier", "destination_type", "trip_month", "total_budget", "monthly_contribution", "current_savings", "latest_trip_month", "minimum_current_breathing_room"];
function orbitPlan(reality, branch = {}) {
  const out = {};
  for (const k of TRAVEL_ORBIT_KEYS) {
    if (branch[k] != null) out[k] = branch[k];
    else if (reality[k] != null) out[k] = reality[k];
  }
  return out;
}
function orbitContext(reality, context = {}) {
  return {
    monthlyIncome: Number(context.monthlyIncome ?? reality.monthly_income) || 0,
    monthlyExpenses: Number(context.monthlyExpenses ?? reality.monthly_expenses) || 0,
    otherGoalsMonthlyOutflow: Number(context.committedExcludingDomain ?? context.otherGoalsMonthlyOutflow ?? context.committedMonthlyTotal) || 0,
    currentSavings: context.currentSavings ?? reality.current_savings ?? null,
    emergencyBufferMonths: context.emergencyBufferMonths ?? null,
    now: context.now ?? undefined,
  };
}

// retirement planData -> the Future-Day Loom plan + context.
function retirementLoomPlan(planData) {
  return {
    future_day: planData.future_day ?? {},
    future_age: planData.future_age ?? null,
    current_age: planData.current_age ?? null,
    monthly_contribution: Number(planData.monthly_contribution) || 0,
    inflation_assumption: planData.inflation_assumption ?? null,
    longevity_years: planData.longevity_years ?? 25,
    real_return_assumption: planData.real_return_assumption ?? null,
    minimum_current_breathing_room: Number(planData.minimum_current_breathing_room) || 0,
  };
}
function retirementLoomContext(planData, context = {}) {
  const cpfLife = Number(planData.target_monthly_income) - Number(planData.gap_monthly);
  return {
    monthlyIncome: Number(planData.monthly_income) || 0,
    monthlyExpenses: Number(planData.monthly_expenses) || 0,
    otherGoalsMonthlyOutflow: Number(context.otherGoalsMonthlyOutflow) || Number(planData.other_goals_monthly_outflow) || 0,
    cpfLifeMonthly: Number.isFinite(cpfLife) && cpfLife > 0 ? cpfLife : null,
    existingRetirementAssets: context.existingRetirementAssets ?? planData.existing_retirement_assets ?? null,
    emergencyBufferMonths: context.emergencyBufferMonths ?? null,
  };
}

// A loan reality/branch planData -> the single-debt `debts` array Debt
// Gravity works on. (The multi-debt picture is assembled in the route.)
function loanPlanToDebts(planData) {
  const balance = Number(planData.loan_amount) || 0;
  if (!(balance > 0)) return [];
  return [
    {
      id: planData.purpose ? `loan:${planData.purpose}` : "loan:primary",
      label: planData.purpose ?? "loan",
      kind: "loan",
      balance,
      annualRatePercent: planData.annual_rate_percent ?? null,
      minimumMonthly: Number(planData.monthly_installment) || 0,
      feeConfirmed: planData.early_repayment_fee != null ? Number(planData.early_repayment_fee) : null,
      provenance: "bank_confirmed",
    },
  ];
}
function loanGravityPlan(planData) {
  return {
    target_debt: planData.purpose ? `loan:${planData.purpose}` : "loan:primary",
    extra_monthly: Number(planData.extra_repayment) || 0,
    one_off_payment: Number(planData.one_off_payment) || 0,
    breathing_room_floor: Number(planData.breathing_room_floor) || 0,
    repayment_strategy: planData.repayment_strategy ?? null,
    excluded_debt_ids: planData.excluded_debt_ids ?? [],
  };
}
function loanGravityContext(planData, context = {}) {
  return {
    monthlyIncome: Number(planData.monthly_income) || 0,
    monthlyExpenses: Number(planData.monthly_expenses) || 0,
    ...
    protectedSavings: Number(context.protectedSavings) || 0,
  };
}
...
const weddingAdapter = {
  ...
  // Living Thread commit 10: aligned to the shared Studio Contract -
  // returns the unified impactSet (ghost/solid, named pressure, nothing
  // auto-routed). The legacy two-layer projectWeddingBranchImpact stays in
  // lib/wedding/cross-goal-projection.js for the existing wedding scene.
  projectImpacts(branchData, realityData, context = {}, allocation = null) {
    return projectWeddingThreadImpact({
      branchPlan: branchData,
      realityPlan: realityData,
      context,
      allocation: allocation ?? branchData.allocation ?? null,
    });
  },
  legacyProjectImpacts(branchData, realityData, context = {}, allocation = null) {
...
const emergencyAdapter = {
  ...
  feasibility(planData, context = {}) {
    ...
    // Safety Runway: the full domain finance (essential burn, current /
    // protected runway, per-commitment survivability, quiet zone).
    const runway = computeSafetyRunway({
      planData,
      context: {
        monthlyExpenses: { value: expenses, provenance: planData.monthly_expenses != null ? "user_confirmed" : "unknown" },
        liquidAssets: { value: planData.current_savings != null ? current : null, provenance: planData.current_savings != null ? "bank_confirmed" : "unknown" },
        essentialShare: planData.essential_share,
    ...
    return {
      ...
      runway: runway.available ? runway : null,
      sealable: runway.available ? runway.sealable : true,
      sealableReason: runway.available ? runway.sealableReason : "rebuild_always_sealable",
  ...
  projectImpacts(branchData, realityData, context = {}, allocation = null) {
    return projectRunwayImpact({
      branchData,
      realityData,
      context: {
        monthlyExpenses: { value: Number(realityData.monthly_expenses) || 0, provenance: realityData.monthly_expenses != null ? "user_confirmed" : "unknown" },
        liquidAssets: { value: realityData.current_savings != null ? Number(realityData.current_savings) : null, provenance: realityData.current_savings != null ? "bank_confirmed" : "unknown" },
        essentialShare: realityData.essential_share,
        commitments: context.commitments ?? [],
      },
      allocation,
    });
  ...
  constraintMetrics(planData, feasibility, context = {}) {
    const runway = feasibility?.runway ?? null;
    return {
      emergency_floor_months: context.emergencyBufferMonths ?? feasibility?.bufferMonths ?? null,
      max_monthly_contribution: context.proposedMonthly ?? null,
      no_guardian_auto_move: context.guardianAutoMove === true,
      // Emergency domain pins
      minimum_floor_months: runway ? runway.currentRunwayMonths : feasibility?.bufferMonths ?? null,
      maximum_rebuild_monthly: Number(planData.monthly_contribution) || 0,
      no_goal_funding_below_floor:
        runway && runway.currentRunwayMonths != null ? runway.currentRunwayMonths < runway.floorMonths && (Number(planData.fund_goals_anyway) === 1) : false,
    };
  },
};
...
const loanAdapter = {
  ...
  feasibility(planData, context = {}) {
    ...
    return {
      ...
      // Debt Gravity: the domain-native view (Gravity Bodies, Release Knot,
      // Freedom Date, Future Handoff preview). Owns the explicit verdict.
      gravity: (() => {
        const g = computeDebtGravity({ debts: loanPlanToDebts(planData), planData: loanGravityPlan(planData), context: loanGravityContext(planData, context) });
  ...
  constraintMetrics(planData, feasibility, context = {}) {
    const g = feasibility?.gravity ?? computeDebtGravity({ debts: loanPlanToDebts(planData), planData: loanGravityPlan(planData), context: loanGravityContext(planData, context) });
    return {
      emergency_floor_months: context.emergencyBufferMonths ?? feasibility?.emergencyMonthsAfter ?? null,
      max_monthly_contribution: context.proposedMonthly ?? null,
      no_guardian_auto_move: context.guardianAutoMove === true,
      // Debt Gravity domain pins
      minimum_breathing_room: g?.available ? g.breathingRoom.value : feasibility?.monthlyFreedom ?? null,
      maximum_extra_payment: Number(planData.extra_repayment) || 0,
      protect_emergency_floor: (context.emergencyBufferMonths ?? 99) < 6 && (Number(planData.extra_repayment) || 0) > 0,
      no_one_off_from_protected_savings: Boolean(g?.available && g.oneOffFromProtectedSavings),
      target_debt_only: Array.isArray(planData.excluded_debt_ids) && planData.excluded_debt_ids.length > 0,
    };
  },
  // Server-owned cross-goal impactSet. Uses Debt Gravity when the route
  // supplies the full `debts` list; otherwise the single confirmed loan.
  projectImpacts(branchData, realityData, context = {}, allocation = null) {
    const debts = Array.isArray(context.debts) && context.debts.length ? context.debts : loanPlanToDebts(realityData);
    return projectDebtImpact({
      branchPlan: { ...loanGravityPlan(realityData), extra_monthly: Number(branchData.extra_repayment) || 0, one_off_payment: Number(branchData.one_off_payment) || 0, target_debt: branchData.target_debt ?? loanGravityPlan(realityData).target_debt },
      realityPlan: loanGravityPlan(realityData),
      debts,
      context: loanGravityContext(realityData, context),
      allocation: allocation ?? branchData.allocation ?? null,
    });
  },
  allocationTargets() {
...
const retirementAdapter = {
  ...
  feasibility(planData, context = {}) {
    ...
    return {
      ...
      // Future-Day Loom: the domain-native view (a Future Day the customer
      // builds, worked backwards to a contribution RANGE). Owns the verdict.
      loom: (() => {
        const l = computeFutureLoom({ planData: retirementLoomPlan(planData), context: retirementLoomContext(planData, context) });
        return l.available ? l : null;
      })(),
  ...
  constraintMetrics(planData, feasibility, context = {}) {
    const l = feasibility?.loom ?? computeFutureLoom({ planData: retirementLoomPlan(planData), context: retirementLoomContext(planData, context) });
    return {
      emergency_floor_months: context.emergencyBufferMonths ?? null,
      max_monthly_contribution: context.proposedMonthly ?? null,
      no_guardian_auto_move: context.guardianAutoMove === true,
      // Future-Day Loom domain pins
      minimum_current_breathing_room: l?.available ? l.currentBreathingRoomAfter.value : feasibility?.contributionToIncome != null ? null : null,
      minimum_emergency_floor: context.emergencyBufferMonths ?? null,
      maximum_monthly_contribution: Number(planData.monthly_contribution) || 0,
      no_assumed_inheritance: Number(planData.assumed_inheritance) > 0,
      no_unconfirmed_partner_assets: Number(planData.unconfirmed_partner_assets) > 0,
    };
  },
  projectImpacts(branchData, realityData, context = {}, allocation = null) {
    return projectFutureDayImpact({
      branchPlan: { ...retirementLoomPlan(realityData), monthly_contribution: Number(branchData.monthly_contribution) || 0, future_day: branchData.future_day ?? retirementLoomPlan(realityData).future_day, future_age: branchData.future_age ?? retirementLoomPlan(realityData).future_age },
      realityPlan: retirementLoomPlan(realityData),
      context: retirementLoomContext(realityData, context),
      allocation: allocation ?? branchData.allocation ?? null,
    });
  },
  allocationTargets() {
...
const travelAdapter = {
  ...
  feasibility(planData, context = {}) {
    ...
    return {
      available: true,
      // Calendar Orbit: the domain-native view (a trip on a 12-month orbit,
      // funding worked backwards to a required monthly pace RANGE). Owns
      // the verdict.
      orbit: (() => {
        const o = computeCalendarOrbit({ planData: orbitPlan(planData, {}), context: orbitContext(planData, context) });
        return o.available ? o : null;
      })(),
  ...
  constraintMetrics(planData, feasibility, context = {}) {
    const o = feasibility?.orbit ?? computeCalendarOrbit({ planData: orbitPlan(planData, {}), context: orbitContext(planData, context) });
    return {
      emergency_floor_months: context.emergencyBufferMonths ?? null,
      max_monthly_contribution: context.proposedMonthly ?? null,
      no_guardian_auto_move: context.guardianAutoMove === true,
      // Calendar Orbit domain pins
      minimum_current_breathing_room: o?.available ? o.currentBreathingRoomAfter.value : null,
      minimum_emergency_floor: context.emergencyBufferMonths ?? null,
      maximum_monthly_contribution: Number(planData.monthly_contribution) || 0,
      latest_trip_month: o?.available ? o.tripMonthInt : null, // YYYYMM int
      no_trip_funding_below_emergency_floor: Boolean(o?.available && (o.liquidityConflict || o.belowBreathing)),
    };
  },
  projectImpacts(branchData, realityData, context = {}, allocation = null) {
    return projectCalendarOrbitImpact({
      branchPlan: orbitPlan(realityData, branchData),
      realityPlan: orbitPlan(realityData, {}),
      context: orbitContext(realityData, context),
      allocation: allocation ?? branchData.allocation ?? null,
    });
  },
  allocationTargets() {
...
const investmentAdapter = {
  ...
  feasibility(planData, context = {}) {
    ...
    return {
      available: true,
      // Capital Prism: the domain-native view (a beam of capital split into
      // spectral jobs, worked to a years-to-target with NO return assumed).
      // Owns the verdict.
      prism: (() => {
        const p = computeCapitalPrism({ planData: prismPlan(planData, {}), context: prismContext(planData, context) });
  ...
  constraintMetrics(planData, feasibility, context = {}) {
    const p = feasibility?.prism ?? computeCapitalPrism({ planData: prismPlan(planData, {}), context: prismContext(planData, context) });
    return {
      emergency_floor_months: context.emergencyBufferMonths ?? feasibility?.emergencyFundMonths ?? null,
      max_monthly_contribution: context.proposedMonthly ?? null,
      no_guardian_auto_move: context.guardianAutoMove === true,
      // Capital Prism domain pins
      minimum_emergency_floor: context.emergencyBufferMonths ?? p?.emergencyFundMonths ?? null,
      minimum_liquid_capital: p?.available ? p.liquidKept.value : null,
      minimum_flexible_capital: p?.available ? p.flexibleAndUnassigned.value : null,
      maximum_monthly_contribution: p?.available ? p.investingCommitment.value : Number(planData.monthly_commitment) || 0,
      no_investing_below_readiness_gate: Boolean(p?.available && p.investingBlockedByGate),
    };
  },
  projectImpacts(branchData, realityData, context = {}, allocation = null) {
    return projectCapitalPrismImpact({
      branchPlan: prismPlan(realityData, branchData),
      realityPlan: prismPlan(realityData, {}),
      context: prismContext(realityData, context),
      allocation: allocation ?? branchData.allocation ?? null,
    });
  },
  allocationTargets() {
...
const insuranceAdapter = {
  ...
  feasibility(planData, context = {}) {
    ...
    return {
      available: true,
      // Living Envelope: the domain-native view (a protection membrane
      // around real life nodes; the monthly premium is the pressure).
      // Owns the verdict.
      envelope: (() => {
        const l = computeLivingEnvelope({ planData: envelopePlan(planData, {}), context: envelopeContext(planData, context) });
        return l.available ? l : null;
      })(),
      nodes: e.nodes,
  ...
  constraintMetrics(planData, feasibility, context = {}) {
    const l = feasibility?.envelope ?? computeLivingEnvelope({ planData: envelopePlan(planData, {}), context: envelopeContext(planData, context) });
    return {
      emergency_floor_months: context.emergencyBufferMonths ?? null,
      max_monthly_contribution: context.proposedMonthly ?? null,
      no_guardian_auto_move: context.guardianAutoMove === true,
      // Living Envelope domain pins
      minimum_current_breathing_room: l?.available ? l.currentBreathingRoomAfter.value : null,
      maximum_monthly_contribution: l?.available ? l.premiumAfter.value : Number(planData.monthly_premium_now) || 0,
      minimum_income_protection_months: l?.available ? l.incomeProtectionMonths : Number(planData.income_protection_months) || null,
      no_underwriting_or_quote: false,
      no_unknown_counted_as_gap: false,
    };
  },
  projectImpacts(branchData, realityData, context = {}, allocation = null) {
    return projectLivingEnvelopeImpact({
      branchPlan: envelopePlan(realityData, branchData),
      realityPlan: envelopePlan(realityData, {}),
      context: envelopeContext(realityData, context),
      allocation: allocation ?? branchData.allocation ?? null,
    });
  },
  allocationTargets() {
...
const familyAdapter = {
  ...
  feasibility(planData, context = {}) {
    ...
    return {
      available: true,
      // Private Constellation: the domain-native view. Two independent
      // participant identities; the other's numbers are never returned.
      // Owns the verdict.
      constellation: (() => {
        const c = computePrivateConstellation({ planData: constellationPlan(planData, {}), viewerKey: context.viewerKey ?? null, context: constellationContext(planData, context) });
  ...
  constraintMetrics(planData, feasibility, context = {}) {
    const c = feasibility?.constellation ?? computePrivateConstellation({ planData: constellationPlan(planData, {}), viewerKey: context.viewerKey ?? null, context: constellationContext(planData, context) });
    return {
      emergency_floor_months: context.emergencyBufferMonths ?? null,
      max_monthly_contribution: context.proposedMonthly ?? null,
      no_balance_share: true, // family always keeps individual balances private
      no_guardian_auto_move: context.guardianAutoMove === true,
      // Private Constellation domain pins
      minimum_current_breathing_room: c?.available ? c.currentBreathingRoomAfter.value : null,
      maximum_monthly_contribution: c?.available ? c.viewerShare.value : Number(planData.shared_monthly_contribution) || 0,
      minimum_confirmations: c?.available ? (c.bothConfirmed ? 2 : (c.otherParticipant?.confirmed ? 1 : 0) + (c.viewerView?.confirmed ? 1 : 0)) : 0,
      no_partner_data_in_viewer_response: false,
    };
  },
  projectImpacts(branchData, realityData, context = {}, allocation = null) {
    return projectPrivateConstellationImpact({
      branchPlan: constellationPlan(realityData, branchData),
      realityPlan: constellationPlan(realityData, {}),
      context: constellationContext(realityData, context),
      allocation: allocation ?? branchData.allocation ?? null,
