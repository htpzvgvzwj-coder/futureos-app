...
export async function recordAppliedAction(hardshipSessionId, { actionType, targetDomain, amount, explanation, status = "applied" }) {
  const result = await query(
    `insert into hardship_actions_applied (hardship_session_id, action_type, target_domain, amount, explanation, status)
     values ($1, $2, $3, $4, $5, $6) returning *`,
    [hardshipSessionId, actionType, targetDomain ?? null, amount ?? null, explanation, status],
  );
...
export async function getAppliedActions(hardshipSessionId) {
  const result = await query(
    `select id, action_type, target_domain, amount, explanation, applied_at, status
