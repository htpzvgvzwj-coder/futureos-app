...
export async function getOrCreateSession(profileKey) {
  const result = await query(
    `insert into hardship_sessions (profile_key) values ($1)
...
// Unlike getLatestArtifact, returns every assessment ever made (with timestamps) -
// the debate-outcome resolver (lib/mirror-outcome-resolver.js) needs the full history
// to check whether a hardship was reported at any point after a given date, not just
// the most recent one.
export async function getAllArtifactsWithTimestamps(sessionId, stage, artifactType) {
  const result = await query(
    `select payload, created_at from hardship_artifacts
     where session_id = $1 and stage = $2 and artifact_type = $3
     order by created_at asc`,
    [sessionId, stage, artifactType],
  );
  return result.rows.map((row) => ({ payload: row.payload, createdAt: row.created_at }));
}
