...
// turn by Anthropic itself; hitting the cap returns stop_reason "pause_turn"
// rather than ending the turn. Resume by resending the assistant's partial
// response — the server continues where it left off. Do NOT add an extra
// user message.
//
// maxPauseIterations bounds this well below Anthropic's own 10 - confirmed
// via a real live call (wedding/stage1, which uses web_search + thinking:
// adaptive + max_tokens 12000) that letting a search-heavy turn run
// unbounded genuinely hit Vercel's 60s serverless function timeout (a real
// 504, not a caught application error - the platform kills the function
// before any try/catch or mock-fallback code gets a chance to run). Safe as
// a global default: routes that don't pass web_search-capable tools never
// return "pause_turn" more than once anyway, so this only changes behavior
// for the routes actually at risk.
const DEFAULT_MAX_PAUSE_ITERATIONS = 4;

export async function runToolTurn(client, params, maxPauseIterations = DEFAULT_MAX_PAUSE_ITERATIONS) {
  const messages = [...params.messages];
  let response = await client.messages.create({ ...params, messages });
  let iterations = 0;
  while (response.stop_reason === "pause_turn" && iterations < maxPauseIterations) {
