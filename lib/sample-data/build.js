...
// Four direction-changing events over the past ~two months, each with a
// real impact_set AND a matching frozen Life Thread snapshot, so Life
// Memory shows a populated timeline and "See your line as it was then"
// works immediately on a freshly loaded account.
async function seedLifeHistory(uid) {
  ...
  const compact = ({ direction, weatherId, weatherLabel, free, committed, safety }) => ({
    ...
    numbers: [
      { id: "free", label: "Free each month", value: `SGD ${free.toLocaleString("en-SG")}` },
      { id: "committed", label: "Promised to your future", value: `SGD ${committed.toLocaleString("en-SG")}/mo` },
      { id: "safety", label: "Safety buffer", value: `${safety.toFixed(1)} months` },
    ],
    nodes: [
      node("income", "Today", "solid", `SGD ${INCOME.toLocaleString("en-SG")}/mo`),
      node("safety", "Safety", "solid", `${safety.toFixed(1)} months`),
      node("relationships", "Wedding", "solid"),
      node("home", "Home", "solid"),
      node("freedom", "Freedom", "solid", "SGD 400/mo"),
  ...
  const steps = [
    {
      daysAgo: 55,
      actor: "user", sourceFeature: "emergency", actionType: "plan_updated", status: "active",
      messageKey: "changeLedger.event.plan_updated.headline",
      cause: { trigger: "future_field_seal", domain: "emergency" },
      relatedGoalIds: ["emergency"],
      impactSet: [
        { goalId: "cashflow", metric: "freeMonthlyCashflow", before: 4100, after: 4050, unit: "sgd", direction: "down" },
        { goalId: "emergency", metric: "bufferMonths", before: 4.2, after: 5.0, unit: "months", direction: "up" },
      ],
      snap: { direction: "Your plans are moving forward and your safety buffer is intact.", weatherId: "calm", weatherLabel: "Calm", free: 4050, committed: 1450, safety: 5.0 },
    },
    {
      daysAgo: 35,
      ...
      impactSet: [
        { goalId: "cashflow", metric: "freeMonthlyCashflow", before: 4050, after: 3600, unit: "sgd", direction: "down" },
        { goalId: "wedding", metric: "readyMonthShift", before: 0, after: -2, unit: "date_shift_months", direction: "up" },
        { goalId: "home", metric: "readyMonthShift", before: 0, after: 1, unit: "date_shift_months", direction: "down" },
      ],
      snap: { direction: "Your plans are moving forward and your safety buffer is intact.", weatherId: "tight", weatherLabel: "Tight", free: 3600, committed: 1900, safety: 4.9 },
    },
    {
      daysAgo: 18,
      actor: "guardian", sourceFeature: "guardian", actionType: "plan_updated", status: "scheduled",
      messageKey: "ledger.collisionPathChosen",
      cause: { trigger: "guardian_collision_path", pathId: "ease_both", competing: ["home", "wedding"] },
      relatedGoalIds: ["home", "wedding"],
      impactSet: [
        { goalId: "cashflow", metric: "freeMonthlyCashflow", before: 3600, after: 3750, unit: "sgd", direction: "up" },
        { goalId: "wedding", metric: "readyMonthShift", before: 0, after: 1, unit: "date_shift_months", direction: "down" },
        { goalId: "home", metric: "readyMonthShift", before: 0, after: 1, unit: "date_shift_months", direction: "down" },
      ],
      snap: { direction: "Your life is protected, but Home and Wedding are beginning to compete for the same money.", weatherId: "recovering", weatherLabel: "Recovering", free: 3750, committed: 1750, safety: 4.9 },
    },
    {
      daysAgo: 5,
      actor: "user", sourceFeature: "home", actionType: "plan_updated", status: "active",
      messageKey: "changeLedger.event.plan_updated.headline",
      cause: { trigger: "future_field_seal", domain: "home" },
      relatedGoalIds: ["home"],
      impactSet: [
        { goalId: "cashflow", metric: "freeMonthlyCashflow", before: 3750, after: 3450, unit: "sgd", direction: "down" },
        { goalId: "home", metric: "readyMonthShift", before: 0, after: -3, unit: "date_shift_months", direction: "up" },
        { goalId: "emergency", metric: "bufferMonths", before: 4.9, after: 4.6, unit: "months", direction: "down" },
      ],
      snap: { direction: "Your plans are moving forward and your safety buffer is intact.", weatherId: "tight", weatherLabel: "Tight", free: 3450, committed: 2150, safety: 4.6 },
    },
  ];

  for (const s of steps) {
    const occurredAt = iso(s.daysAgo);
    // a real, resolvable Change Ledger message key + params
    const dom = s.cause?.domain ?? (s.relatedGoalIds ?? [])[0] ?? "home";
    const res = await recordEventSafe({
      ...
      messageParams: { domain: dom, amount: Math.abs((s.impactSet?.[0]?.after ?? 0) - (s.impactSet?.[0]?.before ?? 0)) },
  ...
  // the baseline
  const base = compact({ direction: "Start a plan and Life shows where your money is taking you.", weatherId: "calm", weatherLabel: "Calm", free: 4100, committed: 0, safety: 4.2 });
  await query(
    `insert into life_thread_snapshots (profile_key, ledger_event_id, kind, event_at, thread, free_monthly, committed_monthly, safety_months)
     values ($1, null, 'baseline', null, $2, 4100, 0, 4.2)
