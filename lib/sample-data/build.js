...
async function seedLifeHistory(uid) {
  ...
  const base = compact({ direction: "Future Bank built your first picture from your accounts, income, liabilities and 6 months of transactions.", weatherId: "calm", weatherLabel: "Calm", free: 3900, committed: 1500, safety: 23.6 });
  await query(
    `insert into life_thread_snapshots (profile_key, ledger_event_id, kind, event_at, thread, free_monthly, committed_monthly, safety_months)
...
export async function buildChildAccount(uid, { wipeFirst = true } = {}) {
  if (wipeFirst) await wipeSampleAccount(uid);

  await savePreferences(uid, { profileVersion: 3, profile: { statedMonthlyIncome: 120, monthlyExpenses: 80, currentSavings: 145 } });
  ...
  tx("credit", 60, 84, "other", "Opening balance", "opening_balance");
  ...
  await createFinancialAsset(uid, { assetClass: "bank_account", label: "My account", linkedAccountId: everyday, currentValue: 145, liquidityClass: "cash", sourceType: "synthetic_fixture" });
  await createAsset(uid, { category: "financial", subtype: "checking_deposit", name: "OCBC My account", value: 145, details: { liquidity: "cash" } });

  // a linked guardian (a parent), so anything that needs a yes has
  // somewhere to go, with a SGD 15/week auto-approve ceiling.
  const g = await grantRole(uid, { subjectKey: `guardian-of-${uid}`, role: "guardian", scope: "approve", relationLabel: "Mum", note: "parent" }).catch(() => null);
...
export async function buildElderAccount(uid, { wipeFirst = true } = {}) {
  if (wipeFirst) await wipeSampleAccount(uid);
  ...
  await grantRole(uid, { subjectKey: `trusted-of-${uid}`, role: "trusted_contact", scope: "suggest", relationLabel: "My daughter", note: "emergency contact" }).catch(() => {});
