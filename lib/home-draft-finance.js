...
export const FIRST_HOME_DOWN_PAYMENT_RATE = 1 - calculateDownPayment(100, { existingLoanCount: 0, loanType: "bank" }).ltv; // 0.25, derived not hardcoded twice
...
// Real ready-date projection for a SPECIFIC monthly amount, factored out
// of computeDownPaymentReadiness so a Moment's "adjust the pace" slider
// (which adjusts a specific savings-plan contribution, not the customer's
// whole income-minus-expenses surplus) can reuse the exact same walk-
// forward math rather than a second, drift-prone copy of it.
export function computeReadyDateForMonthlyAmount({ downPaymentNeeded, currentSavings, monthlyAmount }) {
  const shortfall = downPaymentNeeded - currentSavings;

  if (shortfall <= 0) {
    return { monthsToReady: 0, readyNow: true, readyMonth: null };
  }
  if (monthlyAmount <= 0) {
    return { monthsToReady: null, readyNow: false, readyMonth: null };
  }

  const monthsToReady = Math.ceil(shortfall / monthlyAmount);
  const readyDate = new Date();
  readyDate.setMonth(readyDate.getMonth() + monthsToReady);

  return { monthsToReady, readyNow: false, readyMonth: readyDate.toISOString().slice(0, 7) };
}
...
export function computeDownPaymentReadiness({ targetPrice, currentSavings, monthlyIncome, monthlyExpenses, committedMonthlyTotal }) {
  ...
  const projection = computeReadyDateForMonthlyAmount({ downPaymentNeeded, currentSavings, monthlyAmount: monthlySurplus });
  return { downPaymentNeeded, monthlySurplus, ...projection };
}
