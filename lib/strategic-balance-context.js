...
async function getConfirmedLoans() {
...
async function getConfirmedInvestments() {
  const session = await investmentStore.getOrCreateSession(investmentStore.DEFAULT_PROFILE_KEY);
...
async function getConfirmedSavings() {
  const results = await Promise.all(
...
export async function getStrategicBalanceSnapshot() {
  const [loans, investments, savings, hardshipEvidence] = await Promise.all([
    getConfirmedLoans(),
    getConfirmedInvestments(),
    getConfirmedSavings(),
    getHardshipEvidence(),
  ]);
