...
export function figuresFrom(ct = {}) {
  ...
  return {
    // `free_monthly` is the headline monthly figure — now "available after
    // living costs"; still reads the old `free` id on legacy snapshots.
    free_monthly: money(by.afterLiving ?? by.free ?? by.flexible),
    committed_monthly: money(by.committed),
    safety_months: money(by.safety),
    flexible_monthly: money(by.flexible ?? by.free),
  };
}
...
export function movedBetween(snapshotRow, nowFigures) {
  ...
  const out = [];
  const pairs = [
    ["free_monthly", "Available after living costs", "sgd"],
    ["committed_monthly", "Promised to plans", "sgd"],
    ["flexible_monthly", "Still flexible", "sgd"],
