...
const KEEP = new Set([
  ...
  "commitment_paused", "commitment_resumed", "commitment_reduced",
  "branch_created", "branch_merged", "branch_sealed",
  "guardian_action", "rescue_adopted", "turning_point",
  "asset_added", "liability_added", "income_changed",
...
function whatHappened(ev, domain) {
  const d = domain ? DOMAIN_LABEL[domain] : null;
  switch (ev.action_type) {
    case "plan_created": return `${d ?? "A"} plan started`;
    case "plan_updated": return `${d ?? "A"} plan changed`;
    case "plan_impact": case "turning_point": return `${d ?? "A"} plan shifted`;
    case "commitment_paused": return `${d ?? "A"} plan paused`;
    case "commitment_resumed": return `${d ?? "A"} plan resumed`;
    case "commitment_reduced": return `${d ?? "A"} plan reduced`;
    case "branch_created": return `${d ?? "A"} — a possible path`;
    case "branch_merged": return `${d ?? "A"} paths merged`;
    case "branch_sealed": return `${d ?? "A"} plan confirmed`;
    ...
    case "allocation_set": return `${d ?? "Freed money"} put to work`;
...
function whyItMatters(ev, domain, money) {
  if (ev.uncertainty_note) return ev.uncertainty_note;
  const d = domain ? DOMAIN_LABEL[domain] : "a plan";
  if (money) {
    const dir = money.after > money.before ? "freed up" : "tightened";
    return `It ${dir} the money you have each month, which changes how fast your other plans can move.`;
  }
  switch (ev.action_type) {
    case "commitment_paused": return `${d} stops moving until you resume it — the money it was using is free again.`;
    case "branch_sealed": return `${d} is now a real commitment Guardian protects.`;
    ...
    default: return `It moved ${d} on your line.`;
  }
}

function sourceOf(ev, domain) {
  ...
  if (/^future_field/.test(t || "")) return `You confirmed this in the ${DOMAIN_LABEL[domain] ?? ""} Studio`.trim();
  ...
  return domain ? `The ${DOMAIN_LABEL[domain]} Studio` : "Your Change Ledger";
}
...
function startingPoint({ twin, lifeThread }) {
  ...
  return {
    ...
    detail: parts.length ? `Built from ${parts.join(", ")}.` : "Add an account or a plan and your line begins.",
    confirmed: true,
    reversible: false,
    planVersion: null,
    evidence: null,
...
export function latestMovementLine(memory) {
  ...
  if (r.money) {
    ...
    bits.push(`Your ${r.money.label.toLowerCase()} ${up ? "increased" : "dropped"} by ${amt}`);
  }
  if (r.plansMoved[0]) bits.push(r.plansMoved[0]);
  return { headline: r.what, lines: bits.slice(0, 2), when: r.when };
}
