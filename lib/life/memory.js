...
// Some producers (lib/change-ledger/producers/*.js) store a terse internal
// reason code in uncertainty_note rather than a sentence — everywhere else
// in this file writes real prose, so those codes need translating here too,
// or they'd leak onto the screen verbatim (e.g. "branch_delta_not_numeric").
const UNCERTAINTY_NOTE_MAP = {
  plan_budget_not_yet_quantified: "The plan doesn't have a budget set yet, so there's no number to compare.",
  joint_action_declined_no_state_change: "Your partner declined this, so nothing on your line changed.",
  branch_delta_not_numeric: "This change doesn't reduce to a single number — recorded so you can see it happened.",
  merge_result_pending_recompute: "The merged path is still being recalculated.",
  constraint_shapes_future_plans_not_a_current_delta: "This sets a rule for future plans rather than changing anything today.",
  seal_amount_not_set: "No monthly amount was set when this was sealed.",
  handover_residual_not_computed: "The amount left to hand over hasn't been worked out yet.",
  allocation_kept_flexible_no_goal_moved: "You kept this money flexible rather than sending it to a goal, so nothing moved.",
  rescue_action_not_monetary: "This step didn't move money — recorded so you can see it happened.",
  quote_amount_not_extracted: "No amount could be read from the quote yet.",
  shadow_run_no_amount_delta: "This was a test run — no real amount changed.",
  policy_change_no_direct_financial_delta: "This changes a rule, not a number.",
  mostly_estimates: "Mostly estimates — not yet backed by real data.",
  no_evidence_yet: "No supporting evidence yet.",
  key_evidence_missing: "Some key evidence is still missing.",
};
function readableUncertaintyNote(note) {
  if (UNCERTAINTY_NOTE_MAP[note]) return UNCERTAINTY_NOTE_MAP[note];
  if (note.startsWith("budget_impact_pending:")) {
    const field = humanize(note.slice("budget_impact_pending:".length));
    return `Waiting on ${field} before the budget impact is known.`;
  }
  return note;
}
...
function whyItMatters(ev, money) {
  if (ev.uncertainty_note) return readableUncertaintyNote(ev.uncertainty_note);
  if (ev.action_type === "plan_updated" && ev.cause?.guestCountBefore != null && ev.cause?.guestCountAfter != null) {
    return `Guest count changed from ${ev.cause.guestCountBefore} to ${ev.cause.guestCountAfter}. The monthly amount and the plans linked to it moved with it.`;
  }
  if (money) {
...
function sourceOf(ev) {
  ...
  if (ev.actor === "system") return "Automatic — FutureOS detected it";
...
function startingPoint({ twin, lifeThread }) {
  ...
  return {
    ...
    why: "This is the money picture FutureOS first built for you. Every record above is a change from here.",
    money: null,
    plansMoved: [],
