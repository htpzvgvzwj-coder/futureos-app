...
async function getDomainFollowThrough(domain, store, profileKey) {
...
export async function getFollowThroughSnapshot(profileKey) {
  const [domains, hardshipEvidence] = await Promise.all([
    Promise.all(Object.entries(SAVINGS_STORES).map(([domain, store]) => getDomainFollowThrough(domain, store, profileKey))),
