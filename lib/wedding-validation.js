import { z } from "zod";
import { computeCoreWeddingLineItems, computePaymentSchedule, DETERMINISTIC_LINE_ITEM_CATEGORIES } from "./wedding-finance.js";
...
const VENUE_TIERS = ["budget", "mid_range", "premium", "luxury"];
const VENUE_TYPES = ["hotel", "restaurant", "community"];
const PHOTOGRAPHY_TIERS = ["basic", "mid_range", "premium"];
const ATTIRE_TIERS = ["budget", "mid_range", "premium"];

// Prepends the 3 deterministic (venue/photography/attire) line items computed
// from real Singapore market rate tables, and filters out any AI-supplied
// line item that collides with those categories — the strict:true tool
// schema instructs the model not to include them, but prose instructions
// aren't guaranteed, and silently correcting is friendlier than rejecting
// the whole response for something not the customer's fault.
function attachWeddingFinancials(planOrBudget) {
  const core = computeCoreWeddingLineItems({
    guestCount: planOrBudget.guest_count,
    venueTier: planOrBudget.venue_tier,
    venueType: planOrBudget.venue_type,
    photographyTier: planOrBudget.photography_tier,
    attireTier: planOrBudget.attire_tier,
  });
  const remaining = planOrBudget.line_items.filter(
    (item) => !DETERMINISTIC_LINE_ITEM_CATEGORIES.includes(item.category.toLowerCase())
  );
  return { ...planOrBudget, line_items: [...core, ...remaining] };
}
...
const plan = z
  .object({
    ...
    venue_tier: z.enum(VENUE_TIERS),
    venue_type: z.enum(VENUE_TYPES),
    photography_tier: z.enum(PHOTOGRAPHY_TIERS),
    attire_tier: z.enum(ATTIRE_TIERS),
    line_items: z.array(lineItem).min(0),
    timeline: z.array(timelineItem),
  })
  .transform(attachWeddingFinancials)
...
export const confirmWeddingBudgetSchema = z
  ...
  // Deterministic deposit/progress/balance split (lib/wedding-finance.js) -
  // attached server-side from the final recomputed total_budget, never
  // proposed by the model.
  .transform((budget) => ({
    ...budget,
    payment_schedule: computePaymentSchedule(budget.total_budget, budget.wedding_date),
  }));
