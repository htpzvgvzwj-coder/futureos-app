...
Customer's real financial profile: monthly income SGD ${profile.monthlyIncome}${profile.isIncomeIrregular ? ` (a ${profile.incomeSampleSize}-month smoothed median - income varies month to month, do not treat it as guaranteed stable)` : ""}, monthly expenses SGD ${profile.monthlyExpenses}.
Real liquid savings on hand: SGD ${profile.currentSavings}${profile.liquidSavingsSourcedFromLedger ? " (from the customer's itemized Asset Profile ledger, cash/near-cash/liquid financial assets, already netted against anything already committed elsewhere)" : " (the customer's own stated current savings figure - they haven't itemized an Asset Profile ledger yet, so this is not independently verified against real asset entries)"}. This covers ${profile.emergencyBufferMonths} months of their expenses as an emergency buffer. Active insurance coverage on file: ${profile.hasActiveInsurance ? "yes" : "no"} - a thin buffer or no insurance are real factors to weigh when deciding how aggressively to commit savings toward this down payment.
