...
Real liquid savings on hand (from the customer's actual Asset Profile ledger, cash/near-cash/liquid financial assets, already netted against anything already committed elsewhere): SGD ${profile.currentSavings}. This covers ${profile.emergencyBufferMonths} months of their expenses as an emergency buffer. Active insurance coverage on file: ${profile.hasActiveInsurance ? "yes" : "no"} - a thin buffer or no insurance are real factors to weigh when deciding how aggressively to commit savings toward this down payment.
