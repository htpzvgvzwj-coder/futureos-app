...
export async function applyGoalPause({
  domain,
  hardshipSessionId,
...
// banker can do.
async function applyAuditOnlyAction(actionType, { hardshipSessionId, amount = null, explanation, status, decisionType = "approve", decisionReason = null, proposedAmount = null }) {
  await recordAppliedAction(hardshipSessionId, { actionType, targetDomain: null, amount, explanation, status, decisionType, decisionReason, proposedAmount });
  return { amount, explanation };
}

export function applyDrawdownEmergencyFund({ hardshipSessionId, amount, explanation, decisionType, decisionReason, proposedAmount }) {
  return applyAuditOnlyAction("drawdown_emergency_fund", { hardshipSessionId, amount, explanation, decisionType, decisionReason, proposedAmount });
}

export function applyInvestExcess({ hardshipSessionId, amount, explanation, decisionType, decisionReason, proposedAmount }) {
  return applyAuditOnlyAction("invest_excess", { hardshipSessionId, amount, explanation, decisionType, decisionReason, proposedAmount });
}

export function applyOtherOcbcSupport({ hardshipSessionId, explanation, decisionType, decisionReason, proposedAmount }) {
  return applyAuditOnlyAction("other_ocbc_support", { hardshipSessionId, explanation, status: "pending_review", decisionType, decisionReason, proposedAmount });
}
