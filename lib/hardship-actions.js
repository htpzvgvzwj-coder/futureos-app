...
export async function applyGoalPause({ domain, hardshipSessionId, newMonthlyContribution, explanation }) {
  ...
  await recordAppliedAction(hardshipSessionId, {
    ...
    targetDomain: domain,
    amount: Math.round((currentPlan.monthly_contribution - newMonthlyContribution) * 100) / 100,
    explanation,
  });
  return updatedPlan;
}

// No domain owns the emergency fund balance itself (profile.currentSavings
// is a client-only, localStorage-backed preferences field for every profile
// field in this app, with no server-side counterpart anywhere) - only the
// audit record is server-side. The frontend applies the actual balance
// change via setPreferences, the same mechanism every other profile edit
// already uses.
export async function applyDrawdownEmergencyFund({ hardshipSessionId, amount, explanation }) {
  await recordAppliedAction(hardshipSessionId, {
...
// recorded purely as an audit row rather than inventing a write into a
// domain that doesn't represent it.
export async function applyInvestExcess({ hardshipSessionId, amount, explanation }) {
  await recordAppliedAction(hardshipSessionId, {
    actionType: "invest_excess",
    targetDomain: null,
    amount,
    explanation,
  });
...
export async function applyOtherOcbcSupport({ hardshipSessionId, explanation }) {
  await recordAppliedAction(hardshipSessionId, {
    actionType: "other_ocbc_support",
    targetDomain: null,
    amount: null,
    explanation,
  });
