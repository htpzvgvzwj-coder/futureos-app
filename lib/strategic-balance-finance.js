...
// Same healthy/tight/at_risk enum as the per-category scoreBand
// (app/page.jsx), so the UI never needs two synonymous-but-differently-
// spelled band vocabularies for one screen.
function healthLabel(residualIncome, monthlyIncome) {
  if (residualIncome >= monthlyIncome * 0.1) return "healthy";
  if (residualIncome >= 0) return "tight";
  return "at_risk";
}

// Overall cashflow-utilization snapshot — same healthy/tight/at_risk
// 3-band convention used everywhere else in Strategic Balance, applied here
// to the customer's total committed monthly outflow across every confirmed
// goal at once.
