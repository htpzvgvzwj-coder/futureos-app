...
export async function getOrCreateSession(profileKey) {
  const existing = await query(
    "select * from retirement_sessions where profile_key = $1",
    [profileKey],
  );
  if (existing.rows[0]) return existing.rows[0];

  const inserted = await query(
    "insert into retirement_sessions (profile_key) values ($1) returning *",
    [profileKey],
  );
...
export async function getMessageHistory(sessionId, stage) {
  const result = await query(
    "select role, content from retirement_messages where session_id = $1 and stage = $2 order by seq asc",
    [sessionId, stage],
  );
...
export async function appendMessages(sessionId, stage, messages) {
  ...
  try {
    ...
    const { rows } = await client.query(
      "select coalesce(max(seq), 0) as max_seq from retirement_messages where session_id = $1 and stage = $2",
      [sessionId, stage],
    );
    ...
    for (const message of messages) {
      seq += 1;
      await client.query(
        "insert into retirement_messages (session_id, stage, seq, role, content) values ($1, $2, $3, $4, $5)",
...
export async function saveArtifact(sessionId, stage, artifactType, payload) {
  const result = await query(
    "insert into retirement_artifacts (session_id, stage, artifact_type, payload) values ($1, $2, $3, $4) returning created_at",
...
export async function getLatestArtifact(sessionId, stage, artifactType) {
  const result = await query(
    `select payload from retirement_artifacts
...
export async function getLatestArtifactWithTimestamp(sessionId, stage, artifactType) {
  const result = await query(
    `select payload, created_at from retirement_artifacts
     where session_id = $1 and stage = $2 and artifact_type = $3
     order by created_at desc limit 1`,
    [sessionId, stage, artifactType],
  );
...
export async function addSavingsCheckin(sessionId, { checkinMonth, amount, note }) {
  const result = await query(
    `insert into retirement_savings_checkins (session_id, checkin_month, amount, note)
...
export async function getSavingsCheckins(sessionId) {
  const result = await query(
    `select id, checkin_month, amount, note, created_at from retirement_savings_checkins
...
export async function updateSessionStatus(sessionId, { stage1Status, stage2Status }) {
  ...
  await query(
    `update retirement_sessions set ${sets.join(", ")}, updated_at = now() where id = $${values.length}`,
    values,
  );
}
