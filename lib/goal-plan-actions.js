...
import { recordSavingsPlanConfirmed, recordGoalPlanConfirmed } from "./change-ledger/record-goal-plan.js";
...
const DOMAIN_CONFIG = {
  ...
  travel: { store: travelStore, stage1ArtifactType: "confirmed_budget", crossGoalDomain: "travel" },
};
...
export async function applyGoalPlanJointConfirm(domain, payload, { profileKey }) {
  ...
  // jointEvidence (lib/joint-plan-evidence.js) is propose-time context for
  // the confirming partner's decision, not a real attribute of the
  // confirmed plan itself - stripped before persisting, same as
  // crossGoalCheckInputs, so the resulting confirmed_savings_plan artifact
  // has the exact same real shape whether it arrived via direct-save or a
  // joint confirm.
  const { kind, crossGoalCheckInputs, jointEvidence: _jointEvidence, ...data } = payload;
  ...
  if (kind === "budget" || kind === "plan") {
    const priorPlan = await config.store.getLatestArtifact(session.id, "stage1", config.stage1ArtifactType);
    ...
    // Change Ledger: same event a direct stage1 confirm would produce.
    await recordGoalPlanConfirmed({ profileKey, domain, data, priorData: priorPlan });
    return { kind, data, confirmedAt };
  }
  if (kind === "savings_plan") {
    const priorSavingsPlan = await config.store.getLatestArtifact(session.id, "stage2", "confirmed_savings_plan");
    ...
    // Change Ledger: same event the direct-save stage2 path records, marked
    // isJoint so the cause reads "joint_savings_plan_confirmed".
    await recordSavingsPlanConfirmed({
      ...
      targetCompleteMonth: data.target_complete_month ?? null,
      crossGoalInputs: crossGoalCheckInputs ?? null,
      isJoint: true,
    });
    return { kind, data, confirmedAt };
  }
