...
export async function createBranch(planId, profileKey, { label, baseVersion, data = {}, delta = {}, feasibility = {}, status = "open" }) {
  const result = await query(
    `insert into plan_branches (plan_id, profile_key, label, base_version, data, delta, feasibility, status)
     values ($1,$2,$3,$4,$5,$6,$7,$8) returning *`,
    [planId, profileKey, label, baseVersion, JSON.stringify(data), JSON.stringify(delta), JSON.stringify(feasibility), status],
  );
  return result.rows[0];
}

// Create a branch AND make it the single active moment - one transaction,
// so at no point are there two `active` branches for the plan. The partial
// unique index plan_branches_one_active_per_plan is the backstop: a
// concurrent create-active loses with a 23505.
export async function createActiveBranchAtomic(planId, profileKey, { label, baseVersion, data = {}, delta = {}, feasibility = {} }) {
  return withTransaction(async (tx) => {
    await tx.query(`update plan_branches set status = 'open', updated_at = now() where plan_id = $1 and status = 'active'`, [planId]);
...
// Atomically make ONE branch the plan's active moment (branchId null =
// deactivate all). Demote + FOR UPDATE lock the target + promote, all in
// one transaction. Throws { code } for a bad target; a concurrent
// activate loses with a 23505 on the partial unique index.
export async function setActiveBranchAtomic(planId, branchId, profileKey) {
  return withTransaction(async (tx) => {
    await tx.query(
      `update plan_branches set status = 'open', updated_at = now()
       where plan_id = $1 and status = 'active' and ($2::uuid is null or id <> $2::uuid)`,
      [planId, branchId ?? null],
    );
    if (!branchId) return { activeBranchId: null };
    const cur = await tx.query(`select id, status, plan_id from plan_branches where id = $1 and profile_key = $2 for update`, [branchId, profileKey]);
    const b = cur.rows[0];
    if (!b) {
    ...
    if (["discarded", "merged", "sealed", "withdrawn"].includes(b.status)) {
      const e = new Error("branch_not_activatable");
      e.code = "BRANCH_NOT_ACTIVATABLE";
      e.branchStatus = b.status;
      throw e;
    }
    await tx.query(`update plan_branches set status = 'active', updated_at = now() where id = $1`, [branchId]);
    ...
    await client.query(
      `update plan_constraints set active = false, released_at = now()
       where profile_key = $1 and kind = $2 and scope = $3 and active = true
         and (plan_id is not distinct from $4::uuid)`,
...
export async function getApplicableConstraints(profileKey, { planId = null, domain = null } = {}) {
  const result = await query(
    // $2/$3 are cast explicitly - Postgres can't infer a param's type from
    // `is not null` / `plan_id = $2` alone when $2 may be null.
    `select * from plan_constraints
     where profile_key = $1 and active = true
       and (scope = 'all' or (scope = 'plan' and plan_id = $2::uuid) or (scope = 'domain' and $3::text is not null))
...
export async function getActiveGuardianPolicy(profileKey, planId) {
  const result = await query(
    `select * from guardian_policies where profile_key = $1 and active = true and (plan_id is not distinct from $2::uuid) order by created_at desc limit 1`,
    [profileKey, planId],
  );
  return result.rows[0] ?? null;
}
