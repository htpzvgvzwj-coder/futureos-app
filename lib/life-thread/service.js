...
export async function buildLifeThread(userId) {
  ...
  // Current resolved plan data per domain — used only for the horizon label
  // a Life node shows ("Wedding 2028", "Home 2031"). Best-effort: a missing
  // or dateless plan just yields no label.
  const planVersions = await Promise.all(plans.map((p) => planStore.getCurrentPlanVersion(p.id).catch(() => null)));
  const planDataByDomain = {};
  plans.forEach((p, i) => { planDataByDomain[p.domain] = planVersions[i]?.data ?? null; });
  const yearOf = (s) => (typeof s === "string" && /^\d{4}/.test(s) ? s.slice(0, 4) : null);
  const nodeHorizon = {
    home: yearOf(planDataByDomain.home?.target_complete_month),
    relationships: yearOf(planDataByDomain.wedding?.wedding_date) || yearOf(planDataByDomain.family?.target_month),
    future: planDataByDomain.retirement?.future_age ? `at ${planDataByDomain.retirement.future_age}` : null,
  };
  ...
  // Only warn about the emergency floor once the person has actually set an
  // emergency target — otherwise EMERGENCY_FLOOR_MONTHS is just our default
  // assumption, and it wrongly alarms accounts with no such plan (e.g. a
  // youth account with pocket money).
  const hasEmergencyPlan = plans.some((p) => p.domain === "emergency");
  const turningPoints = deriveTurningPoints({
    sources: {
      emergencyFloor: hasEmergencyPlan
        ? { bufferMonths: asset.emergencyBufferMonths, floorMonths: EMERGENCY_FLOOR_MONTHS }
        : null,
      completions: [],
      paymentMilestones: [],
      budgetGaps: [],
  ...
  // The "relationships" node reads as Wedding when a wedding plan populates
  // it, otherwise Family — so the line says "Wedding 2028", not "Family".
  const relLabel = nodeKnown("wedding") && !nodeKnown("family") ? "Wedding" : nodeKnown("family") && !nodeKnown("wedding") ? "Family" : null;
  const lifeNodes = [
    ...
    nodeFor("home", { value: null, horizon: nodeHorizon.home, known: nodeKnown("home"), moving: domainsWithDraft.has("home"), waiting: false }),
    nodeFor("relationships", { value: null, label: relLabel, horizon: nodeHorizon.relationships, known: nodeKnown("wedding", "family"), moving: domainsWithDraft.has("wedding") || domainsWithDraft.has("family"), waiting: false }),
    ...
    nodeFor("future", { value: null, horizon: nodeHorizon.future, known: nodeKnown("retirement"), moving: domainsWithDraft.has("retirement"), waiting: false }),
  ];
