...
import { buildCanonicalSnapshot, committedExcludingDomain as committedExcluding } from "./canonical-snapshot.js";
...
export async function collectStudioImpacts({
  branchesByPlan,
  planStore,
  threadContext = {},
  commitmentsByPlanId = {},
  ...
  // One baseline for the whole pass. When the caller does not supply a
  // CanonicalMomentSnapshot we build one from threadContext so every
  // projector still shares a single snapshotId.
  const snapshot =
    canonicalSnapshot ??
    buildCanonicalSnapshot({
      ...
      commitmentsByDomain: threadContext.commitmentsByDomain ?? {},
    });
  ...
  const baseCtx = {
    ...
    emergencyBufferMonths: snapshot.emergencyBufferMonths,
    availableMonthlyCashflow: snapshot.availableMonthlyCashflow,
  ...
  for (const { plan, branches } of branchesByPlan) {
    ...
    // Per-domain context off the ONE snapshot: this domain's own active
    // commitment is removed from "committed elsewhere" exactly once, so a
    // Studio never treats its own sealed branch as external pressure.
    const cxd = committedExcluding(snapshot, plan.domain);
    const projCtx = {
      ...baseCtx,
      committedExcludingDomain: cxd,
      otherGoalsMonthlyOutflow: cxd,
    };
  ...
  const { aggregated, invalid, groupCount, conflictGroups, hasBaselineConflict } = aggregateImpactMeasures(measures);

  // Fold the aggregated groups onto life nodes - but keep the unit on
  // every entry so a consumer can never accidentally sum months + sgd. A
  // baseline-conflicted group is carried through as `state: "conflict"`
  // with no numbers, never a pseudo-precise after.
  const nodeImpacts = {};
  for (const grp of aggregated) {
    ...
    (nodeImpacts[nodeId] = nodeImpacts[nodeId] ?? []).push(
      grp.state === "conflict"
        ? { metric: grp.metric, unit: grp.unit, before: grp.before, state: "conflict", invalidReason: grp.invalidReason, direction: "flat", favourable: null }
        : {
            metric: grp.metric,
            unit: grp.unit,
            before: grp.before,
  ...
  return {
    ...
    aggregated,
    groupCount,
    aggregatedConflicts: conflictGroups,
    hasBaselineConflict,
    nodeImpacts,
    resourceLedger,
...
export function enrichCrossGoalEdges(edges, nodeImpacts) {
  return edges.map((e) => {
    ...
    // A baseline-conflicted group carries no magnitude - it can dominate an
    // edge only as an honest "conflict", never as a number.
    const usable = groups.filter((g) => g.state !== "conflict");
    if (usable.length === 0) {
      return { ...e, magnitude: null, unit: groups[0].unit ?? null, impactState: "conflict", groups };
    }
    const rank = (g) => Math.abs(g.state === "solid" ? (g.confirmedDelta ?? 0) : (g.possibleDelta ?? 0));
    const dominant = [...usable].sort((a, b) => rank(b) - rank(a))[0];
    return {
      ...e,
