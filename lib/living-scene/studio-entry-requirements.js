...
export function buildSeedPatch(domain, answers = {}, { exactAmounts = {}, mode = "confirmed" } = {}) {
  ...
  for (const q of req.questions) {
    ...
    if (v.value != null) {
      patch[q.field] = v.value;
      // An answered question keeps its real provenance even in estimate
      // mode - only the UNANSWERED ones become system_estimate defaults.
      provenance[q.field] = v.provenance ?? "user_confirmed";
