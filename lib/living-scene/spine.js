...
// Exactly ONE Moment Outlet is mounted for a given phase. `null` means the
// scene shows only its RealitySummary + native surface with no outlet.
// `memory` deliberately has NO outlet - the Memory drawer is opened only
// when the customer explicitly asks for History / Why.
export const PHASE_OUTLET = {
  reality: null,
  possible: "causal_receipt",
  allocation: "allocation_moment",
  turning_point: "turning_point_moment",
  committed: "sealed_receipt",
  guardian: "guardian_watch",
  memory: null,
};

export function momentForPhase(phase) {
  return Object.prototype.hasOwnProperty.call(PHASE_OUTLET, phase) ? PHASE_OUTLET[phase] : null;
}

// All the outlet ids, for tests that assert only one is ever mounted.
export const MOMENT_OUTLETS = ["causal_receipt", "allocation_moment", "turning_point_moment", "sealed_receipt", "guardian_watch"];
