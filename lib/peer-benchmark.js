...
// Reused by Shadow Account (lib/shadow-account-finance.js) so both features
// single-source the same disclosed guideline rate rather than each keeping
// its own copy of it.
export function getTypicalSavingsRatePercent(monthlyIncome) {
  return SAVINGS_RATE_TYPICAL_PERCENT[getIncomeBucket(monthlyIncome)];
}
