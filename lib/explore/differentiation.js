...
export function receiptFromLedgerEvent(event, t = (k) => k) {
  ...
  const title = t(event.message_key || event.action_type || "a change");
  return buildFutureReceipt({ title: typeof title === "string" ? title : "a change", changes });
}
...
export function traceSecondOrder({ primaryDomain, direction = "earlier", lt = {} } = {}) {
  ...
  for (let hop = 0; hop < 2; hop++) {
    ...
    const effect = edge.to === "safety"
      ...
      : edge.to === "future"
        ? (direction === "earlier" ? "retirement slips a little" : "retirement holds")
        : `feels the pull (${edge.basis})`;
