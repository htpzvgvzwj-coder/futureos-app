...
export function costOfDelay({ domain, monthlyContribution, readyYear, twin = {}, deltas = [-12, -6, 0, 6, 12], downPaymentNeeded = null, currentSavings = null } = {}) {
  ...
  const baseProjection = realProjection ? computeReadyDateForMonthlyAmount({ downPaymentNeeded, currentSavings, monthlyAmount: C }) : null;
  const extraForShift = (d) => {
    if (d === 0 || !realProjection || baseProjection?.monthsToReady == null) return 0;
    // d<0 = ready sooner = fewer months = needs a bigger monthly amount.
    const targetMonths = Math.max(1, baseProjection.monthsToReady + d);
