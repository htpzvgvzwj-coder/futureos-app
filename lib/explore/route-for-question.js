...
// of typed questions.
//
// `fallbackDomain` is which domain to open when no domain is detected in
// the text. It used to be hardcoded to "home", which sent a typed
// question straight to a dead end -- "You do not have a confirmed home
// plan yet" -- on any account whose Home plan isn't sealed, no matter
// what the question was actually about. The caller should pass the
// account's own first active-commitment domain (a domain it actually has
// a reality path for) when one exists.
export function routeForQuestion(q, fallbackDomain = "home") {
  ...
  // the Studio's framed preview.
  return `future_field:${d ?? fallbackDomain ?? "home"}:ask:${encodeURIComponent(q)}`;
}
