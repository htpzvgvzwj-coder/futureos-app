...
export async function buildFinancialTwinBundle(userId) {
  ...
  // Every holding + debt + income row, with its own provenance - so a
  // "Financial Twin" view can show source / updated / confidence per line
  // and never invent a figure for a missing one.
  const holdings = {
    cashAccounts: (balances ?? []).map((b) => ({
      label: b.displayName || b.kind || "Account",
      class: "bank_account",
      value: b.postedBalance ?? 0,
      liquidity: "liquid",
      sourceType: "ledger_derived",
      asOf: today,
      confidence: "high",
    })),
    assets: (assets ?? []).map((a) => ({
      label: a.label || a.assetClass,
      class: a.assetClass,
      value: a.currentValue ?? 0,
      liquidity: a.liquidityClass ?? "liquid",
      sourceType: a.sourceType ?? "user_confirmed",
      asOf: a.asOf ?? null,
      confidence: a.confidence ?? "medium",
    })),
    liabilities: (liabilities ?? []).map((l) => ({
      label: l.label || l.liabilityClass,
      class: l.liabilityClass,
      value: l.currentBalance ?? 0,
      minimumMonthly: l.minimumMonthly ?? null,
      sourceType: l.sourceType ?? "user_confirmed",
      asOf: l.asOf ?? null,
    })),
    incomeStreams: (incomeStreams ?? []).map((i) => ({
      ...
      nextExpectedDate: i.nextExpectedDate ?? null,
      sourceType: i.sourceType ?? "user_confirmed",
      asOf: i.asOf ?? null,
    })),
  };
