...
function accountRowsToAssetsAndLiabilities(balances) {
  const assets = [];
  const liabilities = [];
...
export async function loadFinancialTwinInput(profileKey, { asOf = null } = {}) {
  ...
  const { assets: acctAssets, liabilities: acctLiabilities } = accountRowsToAssetsAndLiabilities(balances);
  ...
  const monthlyRecurringExpense = recurringRows.rows
    ...
    .reduce((s, r) => s + num(r.monthly_amount), 0);
