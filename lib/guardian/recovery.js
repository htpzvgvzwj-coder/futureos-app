...
export function buildRecoveryPlan({ safeToSpend = {}, rescueCases = [], commitments = [] } = {}) {
  ...
  const pauseDetail = pausable.length
  ...
  const steps = [
    {
      ...
      labelKey: "Keep your essential bills covered",
      detail: `${money(bd.nearTermObligations ?? 0)} of bills due before your next income stays first in line.`,
      detailKey: "{v} of bills due before your next income stays first in line.",
      detailParams: { v: money(bd.nearTermObligations ?? 0) },
      needsConfirm: false,
    },
    {
      order: 2,
      ...
      labelKey: "Protect your safety floor",
      detail: `Your emergency buffer (${money(bd.protectedReserve ?? 0)}) is not touched by any of this.`,
      detailKey: "Your emergency buffer ({v}) is not touched by any of this.",
      detailParams: { v: money(bd.protectedReserve ?? 0) },
      needsConfirm: false,
    },
    {
      order: 3,
      ...
      labelKey: "Pause what can wait",
      detail: pauseDetail,
      detailKey: pausable.length ? "{plans} can pause, freeing about {v}/month. Home and Wedding are left alone." : "No plan can be safely paused right now.",
      detailParams: pausable.length ? { plans: pausable.map((p) => cap(p.domain)).join(", "), v: money(pausableFreed) } : null,
    ...
    },
    {
      ...
      labelKey: "Trim repeated or optional spending",
      detail: trimmable.length ? trimmable.map((o) => o.label).join(" · ") : "Nothing obvious to cut from your recent transactions.",
      detailKey: trimmable.length ? null : "Nothing obvious to cut from your recent transactions.",
      needsConfirm: false,
    },
    {
      order: 5,
      ...
      labelKey: recoveryDate ? "Back to normal by" : "Recovery date",
      ...
      detailKey: recoveryDate ? "{d} — when your next income lands. Guardian recalculates after every real change." : "Needs a known next-income date.",
      detailParams: recoveryDate ? { d: recoveryDate } : null,
      needsConfirm: false,
    },
  ];
