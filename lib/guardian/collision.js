...
//
// Pure: feed it the active commitments + the monthly cashflow available
// *for* plans — i.e. income minus essential living costs, BEFORE plan
// commitments are taken out (availableMonthlyCashflow + monthlyCommittedTotal).
...
export function detectCollision({ commitments = [], availableMonthly = null } = {}) {
  ...
  const summaryKey =
    "{big} needs {bigAmt}/month and {small} needs {smallAmt}/month, but only {free}/month is free — a {gap}/month collision.";
  const summaryParams = {
    big: cap(big.domain),
    bigAmt: money(big.monthly),
    small: cap(small.domain),
    smallAmt: money(small.monthly),
    free: money(availableMonthly),
    gap: money(shortfall),
  };
  ...
  return {
    ...
    summaryKey,
    summaryParams,
    summary: summaryKey.replace(/\{(\w+)\}/g, (_, k) => summaryParams[k]),
    paths: [
      {
        id: "pause_smaller",
        label: `Pause ${cap(small.domain)} for now`,
        labelKey: "Pause {d} for now",
        labelParams: { d: cap(small.domain) },
        effect: `Frees ${money(small.monthly)}/month immediately. ${cap(small.domain)} stops moving until you resume it.`,
        effectKey: "Frees {v}/month immediately. {d} stops moving until you resume it.",
        effectParams: { v: money(small.monthly), d: cap(small.domain) },
        target: { domain: small.domain, op: "pause" },
      },
      {
        id: "shrink_larger",
        label: `Make ${cap(big.domain)} smaller`,
        labelKey: "Make {d} smaller",
        labelParams: { d: cap(big.domain) },
        ...
        effectKey: "Drop {d} to {v}/month. It reaches its target later, both plans keep moving.",
        effectParams: { d: cap(big.domain), v: money(Math.max(0, big.monthly - shortfall)) },
        target: { domain: big.domain, op: "reduce", to: round2(Math.max(0, big.monthly - shortfall)) },
      },
      {
        id: "ease_both",
        label: "Lower both plans a little",
        labelKey: "Lower both plans a little",
        labelParams: null,
        effect: `Each drops about ${money(half)}/month. Both keep moving, both reach their target a bit later.`,
        effectKey: "Each drops about {v}/month. Both keep moving, both reach their target a bit later.",
        effectParams: { v: money(half) },
