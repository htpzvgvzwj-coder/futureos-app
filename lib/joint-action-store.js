...
// What THIS user (as the initiator) has proposed to someone else, and its
// real current status - previously nothing exposed this at all, so a
// proposal disappeared into silence the moment it was sent. Includes every
// status (not just pending) so the initiator can see the real outcome, not
// just what's still outstanding.
export async function listInitiatedJointActions(userId) {
  const result = await query(
    `select ja.*, u.display_name as target_display_name, u.email as target_email
     from joint_actions ja
     join users u on u.id = ja.target_user_id
     where ja.initiator_user_id = $1
     order by ja.created_at desc
     limit 50`,
    [userId]
  );
...
export async function declineJointAction(id, userId, reason) {
  const result = await query(
    `update joint_actions set status = 'declined', decline_reason = $3
     where id = $1 and target_user_id = $2 and status = 'pending' returning *`,
