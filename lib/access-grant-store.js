...
// another's data. 'view_and_act' is a storable access_level (see
// scripts/migrate.sql) but createGrant rejects it here - joint write
// permission is a deliberately separate, unbuilt problem.
export async function createGrant({ grantorUserId, granteeUserId, scope, accessLevel, expiresAt = null }) {
  if (accessLevel !== "view") {
    const error = new Error("not_yet_supported");
    error.code = "not_yet_supported";
    throw error;
  }
