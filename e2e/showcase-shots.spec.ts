...
// The production build marks the session cookie `Secure`. Chromium sends it
// over http://127.0.0.1 for page fetches, but Playwright's APIRequestContext
// does not - so any authenticated call in a test must go through the page.
...
async function apiGet(page, path) {
  return page.evaluate(async (p) => {
    const r = await fetch(p, { headers: { "cache-control": "no-cache" } });
    return { ok: r.ok, status: r.status, body: await r.json().catch(() => null) };
  }, path);
}
...
test("Future Bank slice: welcome -> snapshot -> today -> needs -> home -> change receipt (+ reload)", async ({ page }) => {
  ...
  // give Today a real bill + salary so the current has nodes (via the page, so the session cookie is used)
  const accts = await apiGet(page, `/api/bank/accounts`);
  const acctId = accts.body?.accounts?.[0]?.id;
  expect(acctId, JSON.stringify(accts)).toBeTruthy();
  const seedTxn = await apiPost(page, `/api/bank/transactions`, { accountId: acctId, direction: "debit", amount: 1450, category: "housing", merchant: "Landlord GIRO" });
  expect(seedTxn.ok, JSON.stringify(seedTxn)).toBeTruthy();
  ...
  const explainSheet = page.getByRole("dialog");
  await expect(explainSheet.getByText(/how it's worked out/i).first()).toBeVisible();
  ...
  // wait for the horizon to actually render (pace control appears), not just the h1
  await expect(page.getByLabel(/monthly pace/i)).toBeVisible({ timeout: 20000 });
  await expect(page.getByRole("button", { name: /apply this pace/i })).toBeVisible();
  await page.screenshot({ path: `${OUT}/fb-5-home-horizon.png`, fullPage: true });

  // F - Change Receipt (Money Current ripple) - nudge the pace slider up
  const pace = page.getByLabel(/monthly pace/i);
  await pace.focus();
  ...
  // reload keeps the seeded path (fetch from the page so the session cookie is used)
  await page.reload();
  ...
  expect(field.hasRealityPath, JSON.stringify(field)).toBeTruthy();
...
test("Fix a money problem opens a real Problem sheet, not Today", async ({ page }) => {
  ...
  // either a found case or an honest "no matching issue" - never blank, never routed to Today
  const problemSheet = page.getByRole("dialog");
  await expect(problemSheet).toBeVisible();
  await expect(problemSheet.getByText(/no matching issue is currently found|at risk:/i).first()).toBeVisible();
  await expect(page.getByText(/available to spend/i)).toHaveCount(0);
});
